...
export default function DatasetReviewPage() {
  ...
  async function exportTrainingReport(job: PreprocessingJob) {
    setJobMessage(null); setJobError(null); setTrainingReport(null);
    try {
      const user = getFirebaseServices()?.auth.currentUser;
      ...
      const body = await response.json().catch(() => ({})) as TrainingReadinessReport & { error?: string; reportId?: string };
      if (!response.ok) throw new Error(body.error || "สร้าง training readiness report ไม่สำเร็จ");
      setTrainingReport(body);
      ...
      setJobMessage(body.ready ? `dataset พร้อมตรวจขั้นต่อไปแล้ว (${body.reportId})` : `พบ ${body.blockers?.length ?? 0} จุดที่ต้องแก้ — แสดงรายงานด้านล่างแล้ว`);
