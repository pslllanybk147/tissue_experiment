...
export default function KnowledgePage() {
  ...
  useEffect(() => { if (session.status !== "authenticated" && session.status !== "demo") return; let active = true; Promise.all([repository.list(ownerId), sourceRepository.listSources(ownerId), sourceRepository.listClaims(ownerId)]).then(async ([nextRecords, nextSources, nextClaims]) => { const nextAudits = (await Promise.all(nextClaims.map(claim => sourceRepository.listClaimAuditEvents(ownerId, claim.id)))).flat(); if (active) { setRecords(nextRecords); setSources(nextSources); setClaims(nextClaims); setClaimAudits(nextAudits); setState("ready"); } }).catch(() => { if (active) setState("error"); }); return () => { active = false; }; }, [ownerId, repository, session.status, sourceRepository]);
  async function refreshSources() { const [nextSources, nextClaims] = await Promise.all([sourceRepository.listSources(ownerId), sourceRepository.listClaims(ownerId)]); const nextAudits = (await Promise.all(nextClaims.map(claim => sourceRepository.listClaimAuditEvents(ownerId, claim.id)))).flat(); setSources(nextSources); setClaims(nextClaims); setClaimAudits(nextAudits); }
