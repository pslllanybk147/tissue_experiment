...
function adapter(db: Firestore, uid: string): KnowledgeSourceRepository {
  ...
  return {
    ...
    async createClaim(ownerId, input: SourceClaimInput) { if (ownerId !== uid) throw new Error("Owner mismatch"); if (!input.evidenceExcerpt?.trim()) throw new Error("Evidence excerpt required"); const timestamp = new Date().toISOString(); const claim: SourceClaim = { ...input, id: `claim-${crypto.randomUUID()}`, ownerId: uid, reviewState: "Pending review", reviewerNote: "", reviewedBy: null, reviewedAt: null, createdAt: timestamp, updatedAt: timestamp }; await setDoc(doc(claimCollection, claim.id), claim); return claim; },
