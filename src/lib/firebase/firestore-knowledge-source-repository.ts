...
function adapter(db: Firestore, uid: string): KnowledgeSourceRepository {
  ...
  return {
    ...
    async createSource(ownerId, input: KnowledgeSourceInput) { if (ownerId !== uid) throw new Error("Owner mismatch"); const timestamp = new Date().toISOString(); const source: KnowledgeSource = { ...input, id: `source-${crypto.randomUUID()}`, ownerId: uid, createdAt: timestamp, updatedAt: timestamp }; await setDoc(doc(sourceCollection, source.id), source); return source; },
    ...
    async createClaim(ownerId, input: SourceClaimInput) { if (ownerId !== uid) throw new Error("Owner mismatch"); const timestamp = new Date().toISOString(); const claim: SourceClaim = { ...input, id: `claim-${crypto.randomUUID()}`, ownerId: uid, reviewState: "Pending review", reviewerNote: "", reviewedBy: null, reviewedAt: null, createdAt: timestamp, updatedAt: timestamp }; await setDoc(doc(claimCollection, claim.id), claim); return claim; },
