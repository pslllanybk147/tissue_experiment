...
export function sanitizePlantForFirestore<T>(value: T): T {
  if (Array.isArray(value)) {
    return value.map((item) => sanitizePlantForFirestore(item)) as T;
  }
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value)
        .filter(([, item]) => item !== undefined)
        .map(([key, item]) => [key, sanitizePlantForFirestore(item)]),
    ) as T;
  }
  return value;
}
...
function adapter(db: Firestore, uid: string): PlantRepository {
  ...
  return {
    ...
    async create(ownerId, input) { if (ownerId !== uid) throw new Error("Owner mismatch"); const timestamp = new Date().toISOString(); const item = sanitizePlantForFirestore<PlantRecord>({ ...input, id: "plant-" + crypto.randomUUID(), ownerId: uid, createdAt: timestamp, updatedAt: timestamp }); await setDoc(ref(item.id), item); return item; },
    async update(ownerId, plantId, input: PlantInput) { if (ownerId !== uid) throw new Error("Owner mismatch"); const current = await this.get(ownerId, plantId); if (!current) throw new Error("Plant not found"); const item = sanitizePlantForFirestore<PlantRecord>({ ...input, id: plantId, ownerId: uid, createdAt: current.createdAt, updatedAt: new Date().toISOString() }); await setDoc(ref(plantId), item); return item; },
  };
}
