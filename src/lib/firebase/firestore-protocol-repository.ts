...
export function createFirestoreProtocolRepository(uid: string, options: Options = {}): ProtocolRepository {
  ...
  return {
    ...
    async createDraft(ownerId, input) { guard(ownerId); const timestamp = now(); const id = `protocol-${createId()}`; const versionId = `version-${createId()}`; const protocol: ProtocolRecord = { id, ownerId, title: input.title, slug: input.title.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "protocol", plantScope: input.plantScope, evidenceState: input.evidenceState, status: "Draft", currentVersionId: versionId, createdAt: timestamp, updatedAt: timestamp, deletedAt: null }; const version: ProtocolVersion = { id: versionId, protocolId: id, ownerId, version: "0.1.0", summary: input.summary, changeNote: input.changeNote, steps: removeUndefined(input.steps), claimIds: removeUndefined(input.claimIds ?? []), sourceIds: removeUndefined(input.sourceIds ?? []), createdBy: ownerId, createdAt: timestamp, publishedAt: null }; await adapter.commitMutation({ protocol, version, audit: audit(id, "created", null, protocol) }); return protocol; },
    async createDraftFromClaim(ownerId, claim: SourceClaim, source: KnowledgeSource) { guard(ownerId); const input = createPlaybookDraftInput(claim, source); const timestamp = now(); const id = `protocol-${createId()}`; const versionId = `version-${createId()}`; const protocol: ProtocolRecord = { id, ownerId, title: input.title, slug: input.title.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "protocol", plantScope: input.plantScope, evidenceState: input.evidenceState, status: "Draft", currentVersionId: versionId, createdAt: timestamp, updatedAt: timestamp, deletedAt: null }; const version: ProtocolVersion = { id: versionId, protocolId: id, ownerId, version: "0.1.0", summary: input.summary, changeNote: input.changeNote, steps: [], claimIds: removeUndefined(input.claimIds ?? []), sourceIds: removeUndefined(input.sourceIds ?? []), createdBy: ownerId, createdAt: timestamp, publishedAt: null }; await adapter.commitMutation({ protocol, version, audit: audit(id, "created_from_claim", null, { protocol, version }) }); return protocol; },
    async saveDraftVersion(ownerId, id, versionId, input) { guard(ownerId); const protocol = await required(id); const current = (await adapter.listVersions(id)).find(item => item.id === versionId); if (!current) throw new Error("Protocol version not found"); if (current.publishedAt) throw new Error("Published versions are immutable"); const version = { ...current, summary: input.summary, changeNote: input.changeNote, steps: removeUndefined(input.steps), claimIds: removeUndefined(input.claimIds ?? []), sourceIds: removeUndefined(input.sourceIds ?? []) }; await adapter.commitMutation({ protocol: { ...protocol, title: input.title, plantScope: input.plantScope, evidenceState: input.evidenceState, updatedAt: now() }, version, audit: audit(id, "updated", current, version) }); return version; },
    async createDraftVersion(ownerId, id, sourceVersionId, changeNote) { guard(ownerId); const protocol = await required(id); const source = (await adapter.listVersions(id)).find(item => item.id === sourceVersionId); if (!source) throw new Error("Protocol version not found"); const timestamp = now(); const version: ProtocolVersion = { ...structuredClone(source), id: `version-${createId()}`, version: nextDraftVersion(source.version), changeNote, createdAt: timestamp, createdBy: ownerId, publishedAt: null }; const next = { ...protocol, status: "Draft" as const, currentVersionId: version.id, updatedAt: timestamp }; await adapter.commitMutation({ protocol: next, version, audit: audit(id, "version_created", source, version) }); return version; },
    async activateVersion(ownerId, id, versionId) { guard(ownerId); const protocol = await required(id); const current = (await adapter.listVersions(id)).find(item => item.id === versionId); if (!current) throw new Error("Protocol version not found"); if (protocol.status === "Active" && protocol.currentVersionId === versionId && current.publishedAt) return protocol; validateProtocolForPublish(current); const timestamp = now(); const next = { ...protocol, status: "Active" as const, currentVersionId: versionId, updatedAt: timestamp }; const version = { ...current, publishedAt: timestamp }; await adapter.commitMutation({ protocol: next, version, audit: audit(id, "activated", protocol, next) }); return next; },
