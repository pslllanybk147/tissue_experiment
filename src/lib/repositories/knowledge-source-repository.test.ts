...
describe("knowledge source repository", () => {
  it("keeps new claims pending until review", async () => {
    ...
    const claim = await repository.createClaim("owner-1", { sourceId: source.id, taxonId: "cultivar-pink-princess", category: "tissue-culture", statement: "Example claim", evidenceExcerpt: "Supporting excerpt", evidenceState: "Adapted" });
