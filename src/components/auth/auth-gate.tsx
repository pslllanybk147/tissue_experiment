...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (session.status === "unconfigured") return <main className="auth-screen"><div className="auth-card"><span className="auth-mark">PL</span><p className="eyebrow">SECURE WORKSPACE</p><h1>Firebase ยังไม่ได้ตั้งค่า</h1><p>เพิ่ม Firebase configuration ใน environment ของระบบเพื่อใช้บัญชีจริง หรือเปิดพื้นที่สาธิตเพื่อทดสอบ workflow โดยไม่เขียนข้อมูลขึ้น cloud</p><button className="primary-button auth-action" onClick={useDemo}>Continue in demo mode</button><small>Demo data จะอยู่ในหน่วยความจำและหายเมื่อ refresh</small></div></main>;
