...
export function PreprocessingJobs({ jobs, onStart, onRetry, onExport }: PreprocessingJobsProps) {
  return <section className="preprocessing-jobs" aria-labelledby="preprocessing-jobs-heading">
    ...
    {!jobs.length ? <div className="dataset-empty"><h3>ยังไม่มี preprocessing job</h3><p>อนุมัติ provenance และ label ก่อน แล้วกดเริ่มเพื่อสร้างชุดภาพที่พร้อมใช้</p></div> : <div className="preprocessing-job-list">{jobs.map(job => {
      ...
      return <article className="preprocessing-job" key={job.id}>
        ...
        <div className="preprocessing-job-actions">{failed > 0 && <button className="secondary-button" onClick={() => void onRetry(job)} type="button">Retry รายการที่ล้มเหลว</button>}{job.status === "completed" && failed === 0 && <button className="secondary-button" onClick={() => void onExport(job)} type="button">ดาวน์โหลด model-ready manifest</button>}</div>
