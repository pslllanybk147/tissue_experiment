...
export function ReviewQueue({ items, onReviewProvenance, onSetLabel, onExport }: ReviewQueueProps) {
  ...
  return <div className="dataset-review" aria-label="Dataset review queue">
    ...
    <div className="dataset-review-filters" role="tablist" aria-label="กรองสถานะ">{statuses.map(status => <button aria-selected={filter === status} className={`cl-filter-tab${filter === status ? " active" : ""}`} key={status} onClick={() => setFilter(status)} role="tab" type="button">{status}</button>)}</div>
