...
export function ReviewQueue({ items, onReviewProvenance, onSetLabel, onExport }: ReviewQueueProps) {
  ...
  return <div className="dataset-review" aria-label="Dataset review queue">
    ...
    {!visibleItems.length ? <div className="dataset-empty"><h3>ยังไม่มีรายการในคิวนี้</h3><p>รูปจะเข้าคิวหลังคุณเปิด Lot บันทึก Observation และอัปโหลดรูปพร้อมที่มา</p><ol><li>เลือก Lot ที่ต้องการ</li><li>เพิ่ม Observation และอัปโหลดรูป</li><li>กลับมาหน้านี้เพื่อตรวจที่มาและยืนยัน label</li></ol><div className="route-actions"><Link className="primary-button" href="/experiments">ไปเพิ่มรูปใน Experiment</Link><Link className="quiet-button" href="/plants/new">ยังไม่มีต้นไม้ — เริ่มที่นี่</Link></div></div> : <div className="dataset-review-grid"><div className="dataset-review-list" aria-label="รายการภาพ">{visibleItems.map(item => <button className={`dataset-review-row ${selected?.id === item.id ? "active" : ""}`} key={item.id} onClick={() => selectItem(item)} type="button"><img alt="" src={item.assetUrl} /><span><strong>{item.label?.cultivarName || "ยังไม่ติด label"}</strong><small>{item.lotId} · {item.provenance.kind === "user-captured" ? "ภาพจากผู้ใช้" : "ภาพอ้างอิง"}</small><small>{formatDate(item.createdAt)}</small></span><span className={statusClass(item.reviewStatus)}>{item.reviewStatus}</span></button>)}</div>
