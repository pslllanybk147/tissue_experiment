...
## 2026-08-14 — ตรวจโหมดง่ายบนแอปจริง และยืนยันว่าเครื่องคำนวณไม่ถูกกระทบ

- ผู้ใช้แจ้งว่า "ไม่เห็นมีให้เลือกโหมดง่าย" ตรวจแล้วพบว่าตัวเลือกมีอยู่จริง เปิด `npm run dev` แล้วเข้า
  `/my/rounds/new?slug=pink-princess` เห็นขั้นที่ 1 "รอบนี้จะทำแนวไหน" พร้อมการ์ดสองใบครบ
  สาเหตุคือตอนนั้นยัง**ไม่ได้ push** master ขึ้น origin เว็บ production จึงยังเป็นโค้ดเก่า push แล้ว (`c22a47e..935d0da`)
- บทเรียน: merge เข้า master ไม่พอ ถ้าผู้ใช้ดูจาก Vercel ต้อง push ด้วย และควรบอกให้ชัดว่ายังไม่ได้ push
- ตรวจว่าโหมดง่ายกระทบเครื่องคำนวณไหม คำตอบคือ**ไม่กระทบเลย** `applySimpleMode` แตะแค่ `measurement.required`
  กับ `evidenceRequirement` ส่วนจำนวนกระปุก ปริมาตรต่อกระปุก % เผื่อสูญเสีย อัตราฉลาก MS/BCD และ stock ฮอร์โมน
  ทั้งหมดเป็น state ของ `MediumCalculator` เอง ไม่ได้อ่านจาก measurements
- ยืนยันเพิ่มว่า `medium-volume` กับ `medium-ph` เป็นช่องบันทึกล้วน ไม่มีจุดไหนในระบบคำนวณจากสองค่านี้
  (grep ทั้ง src นอกไฟล์เทสต์แล้วเจอแค่ที่ประกาศ) การปลดจากการบังคับในโหมดง่ายจึงไม่ทำให้สูตรใดเพี้ยน
- ตัวเลขจริงของรอบ `pink-princess` เส้นทางไฮเตอร์เหมือนกันทั้งสองโหมด: 15 ขั้น, 98 การ์ดคำสั่ง, 18 ช่องกรอก
  ต่างกันที่ช่องบังคับ **full = 14 ช่อง, simple = 3 ช่อง** (active-chlorine-percent, sterilize-minutes,
  rinse-actual-ppm ซึ่งอยู่ในขั้นฟอกทั้งหมด)
- **ข้อควรระวังที่พบและยังไม่ได้แก้ (มีมาก่อนโหมดง่าย มีทั้งสองโหมด)**: ช่อง "ปริมาตรอาหาร batch นี้" ใน
  `chemical-preparation.tsx` รับค่า `defaultFinalVolumeMl` ผ่าน `useState` initializer ซึ่งทำงานครั้งเดียว
  ไม่มี effect sync ตามทีหลัง ถ้าผู้ใช้เปลี่ยนจำนวนกระปุกหลังฟอร์มนี้ mount แล้ว ปริมาณไฮเตอร์จะยังคิดจาก
  ปริมาตรเก่าจนกว่าจะพิมพ์ทับเอง ลำดับที่ปลอดภัยคือตั้งจำนวนกระปุกให้จบก่อนแล้วค่อยกรอกฟอร์มเตรียมสาร
- ไม่มีการแก้โค้ดในรอบนี้ เป็นการตรวจสอบล้วน working tree สะอาด
...
- สาเหตุ: `bracket-notice.tsx` render `dose.durationMin[0]` ถึง `dose.durationMin[1]` ตรง ๆ เสมอ ไม่เช็คว่าต้น-ปลาย
  เท่ากันหรือไม่ ค่า `durationMin: [1, 1]` ของ NaDCC rinse ใน `violin-variegated.ts` (แปลว่า "1 นาทีเป๊ะ" ไม่ใช่ช่วง)
  จึงกลายเป็นข้อความซ้ำคำ
- เพิ่ม `formatDurationMinRange()` ใน `bracket.ts` (จุดรวมของ dose-related logic อยู่แล้ว) ยุบเหลือค่าเดียวเมื่อ
  ต้น-ปลายเท่ากัน แก้ไว้ที่จุดกลาง ไม่ใช่เฉพาะจุดที่เจอ เพราะสกุลอื่นที่มี dose bracket (dendrobium, monstera,
  zingiber) อาจเจอกรณีนี้ได้อีกถ้ามีใครตั้งค่า low=high ในอนาคต
- ทดสอบจริงผ่านเบราว์เซอร์: เปิดขั้นฟอกฆ่าเชื้อของ violin-variegated เห็นข้อความเปลี่ยนจาก "1 ถึง 1 นาที"
  เป็น "1 นาที" ถูกต้อง
- Verification รอบนี้:
  - `npm test`: ผ่าน 567 tests (เพิ่มจาก 564 เดิม 3 เทสใหม่), skip 10
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน

## 2026-08-09 — ตรวจทั้งระบบหาจุดที่บอก "ใช้อะไร ปริมาณเท่าไหร่ นานเท่าไหร่" ไม่ครบ แล้วแก้

- ผู้ใช้ชี้ว่าขั้นทำอาหารบอก "ปรับ pH ให้อยู่ในช่วงของสูตร" โดยไม่บอกตัวเลข และขอให้ตรวจทั้งระบบหาปัญหาแบบเดียวกัน
  เกณฑ์ที่ผู้ใช้ตั้งไว้: เปิดขั้นมาแล้วต้องรู้ทันทีว่าใช้อะไร ปริมาณเท่าไหร่ นานแค่ไหน และตอนไหน
- สั่ง Explore agent ตรวจทั้ง core-steps.ts, genera/*.ts, species/*.ts และ layout ของ step-runner.tsx/step-detail.tsx
  หาช่องว่างประเภทนี้ พบ 4 กลุ่มเรียงตามผลกระทบ ต่อไปนี้คือสิ่งที่แก้แล้วในรอบนี้ทั้งหมด:
- **บั๊กร้ายแรงกว่าที่คิด — ตัวเลขผิดจริง ไม่ใช่แค่คลุมเครือ**: `genera/dendrobium.ts`, `genera/monstera.ts`, `genera/zingiber.ts`
  มี `doses["sterilize.dose"]` ที่ตั้งช่วงไว้ต่างจากค่าตั้งต้นของทรง (เช่น dendrobium 0.6–2%, zingiber 1–2.5%)
  แต่ไม่ได้ override `actions` จึงสืบทอดข้อความจาก core-steps.ts ที่บอกเลขของทรง (0.5–1.0%) มาทับ — ผู้ใช้จะเห็น
  ตัวเลขที่ผิดจากงานวิจัยจริงของสกุลนั้นเป็นข้อความแรกที่อ่าน ก่อนจะเจอตารางทดสอบช่วงที่ถูกต้องอยู่ท้ายขั้น (ถ้าเจอเลย
  เพราะ Guided Runner ไม่โชว์ BracketNotice เหมือนหน้าคู่มืออ่านอย่างเดียว) เพิ่ม `actions` override ให้ทั้งสามไฟล์
  ระบุตัวเลขและเวลาที่ตรงกับ dose bracket จริงของแต่ละสกุล
- **จุดตายที่กระทบทุกพันธุ์ — เครื่องคำนวณอาหารหายไปหลังขั้นแรก**: `MediumCalculator` เดิมเช็ค `step.id === "prep-media"`
  เท่านั้น ขั้น `multiply`/`root` ที่มีสูตรอาหารของตัวเอง (`multiplication`/`rooting` ใน `mediaRecipes`) ไม่มีเครื่อง
  คำนวณเลย ผู้ใช้ต้องย้อนกลับไปขั้นแรกที่ผ่านมาหลายสัปดาห์แล้วเอง ย้าย `MEDIUM_CALCULATOR_STEP_IDS` และ
  `initialRecipeIdForStep` ไปไว้ที่ `src/lib/rounds/medium-steps.ts` (ไฟล์ธรรมดา ไม่ใช่ "use client" เพราะ
  step-detail.tsx เป็น server component นำเข้าค่าจากไฟล์ client ไม่ได้ — พังตอน build แบบเงียบจนกว่าจะ prerender จริง
  เจอจาก build error `MEDIUM_CALCULATOR_STEP_IDS.has is not a function`) แล้วให้ทั้ง step-runner.tsx และ
  step-detail.tsx โชว์เครื่องคำนวณในสามขั้นนี้ เลือกสูตรเริ่มต้นให้ตรงขั้นอัตโนมัติ
- **ปัญหาที่ผู้ใช้รายงาน — pH ต้องเลื่อนหาไกล**: ย้าย `MediumCalculator` จากท้ายหน้า (หลัง materials/actions/pass-stop)
  มาไว้ก่อนคำสั่งลงมือทำ ในทั้งสองไฟล์ ตอนนี้ผู้ใช้เห็นตัวเลข pH จริงก่อนจะอ่านประโยคที่อ้างถึงมัน
- **ไม่เคยมีที่ไหนบอก "นานแค่ไหน"**: ทุกขั้นมี `durationMinutes` อยู่แล้วในข้อมูลแต่ไม่เคยถูก render เลยที่ไหน
  เพิ่ม `src/lib/manual/duration.ts` (`formatDurationMinutes`) แสดงเป็นแบดจ์ข้าง EvidenceBadge ทั้งสองหน้า
  ระหว่างตรวจเจอว่าการทำตัวเลขนี้ให้เห็นชัดเผยความขัดแย้งที่ซ่อนอยู่: ขั้น `root` ของ violin-variegated ใช้ค่าเริ่มต้น
  ของทรง (14 วัน) ทั้งที่ evidence.note ของพันธุ์เองบอกว่า "บ่มหนึ่งเดือน" (30 วัน) และขั้น `multiply` ของ java-fern
  ก็ใช้ 14 วันทั้งที่งานตรงพันธุ์บ่มจริง 8 สัปดาห์ (56 วัน) ก่อนนับผลที่ 6 สัปดาห์ — แก้ทั้งสองจุดด้วย `durationMinutes`
  override ให้ตรงกับ evidence จริงของแต่ละพันธุ์ พร้อมแก้ข้อความ action/stopCondition ของ violin-variegated root
  จาก "เมื่อครบกำหนด" เป็น "เมื่อครบหนึ่งเดือน" ให้ตรงกัน
- แก้คำที่กำกวมอีกจุดที่ผู้ใช้ชี้: `core-steps.ts` ขั้น prep-media action 4 เขียนว่า "เกลือ" เฉย ๆ ทั้งที่รายการ
  ของที่ต้องเตรียมเขียนว่า "MS basal salts" อ่านแล้วเข้าใจผิดเป็นเกลือแกงได้ แก้ให้ตรงกัน (คอมมิตแยกก่อนหน้ารอบนี้)
- ทดสอบจริงผ่านเบราว์เซอร์ (guide อ่านอย่างเดียว ไม่ต้อง auth): เปิดขั้น prep-media ของ violin-variegated เห็น
  แบดจ์ "1 ชั่วโมง" และเครื่องคำนวณ (พร้อม pH 5.7-5.8) อยู่ก่อนคำสั่งลงมือทำ → เปิดขั้น multiply เห็นเครื่องคำนวณเลือก
  สูตร "ระยะเพิ่มจำนวนยอด" อัตโนมัติ พร้อมแบดจ์ "14 วัน" → เปิดขั้น root เห็นสูตร "ระยะออกราก" อัตโนมัติ พร้อมแบดจ์
  "30 วัน" (แก้ถูกต้องแล้ว ตรงกับ evidence.note)
- ไม่ได้แตะ: dendrobium/zingiber เป็น genus ที่ยังไม่มี species pack ใดอ้างถึงจริง (ไม่มีหน้า guide ที่เปิดได้)
  ตรวจความถูกต้องผ่านการอ่านโค้ดและเข้าใจ resolve.ts (override แทนที่ทั้ง field เมื่อมี ไม่ merge ลึกกว่านั้น)
  แทนที่จะเปิดหน้าเว็บจริง เพราะไม่มีหน้าให้เปิด
- Verification รอบนี้:
  - `npm test`: ผ่าน 564 tests (เพิ่มจาก 560 เดิม 4 เทสใหม่ ส่วนใหญ่จาก duration.test.ts), skip 10
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน (พบและแก้ปัญหา client/server boundary ระหว่างทางก่อน build ผ่าน)

## 2026-08-09 — เพิ่ม T3 (NaDCC เดี่ยวแทน Haiter) และแบนเนอร์วิธีฆ่าเชื้อในขั้นฟอกฆ่าเชื้อ

- ต่อจากรอบก่อนหน้าในวันเดียวกัน (สร้างระบบชุดทดลอง 4 แขนง) ผู้ใช้ขอให้ทำสองอย่างที่ deferred ไว้: T3 กับแบนเนอร์
- เพิ่ม `SterilizationMethod` ใหม่ `"nadcc-soak"` (คนละอันกับ `RinseWaterMethod = "nadcc"` ซึ่งเป็นน้ำ rinse
  เสริมหลัง Haiter — nadcc-soak แทนขั้น Haiter ทั้งขั้น ไม่ใช่ rinse เสริม)
- T3 arm: NaDCC 300 ppm แช่ 24-48 ชม. แทน Haiter ทั้งขั้น ไม่มี `rinseWater` (ล้างด้วยน้ำปลอดเชื้อธรรมดาต่อตามปกติ
  ตามที่ new_idea.md หัวข้อ 15 ระบุ) ไม่มีช่องเก็บ "ระยะเวลาแช่" ในโครงสร้างข้อมูล (LotSterilizationSnapshot ไม่มี
  field ระยะเวลา) ตัวเลข 24-48 ชม. จึงอยู่ในข้อความ armLabel/แบนเนอร์เท่านั้น ไม่ใช่ตัวเลขในโครงสร้าง
  เพิ่ม `T3_RISK_NOTE` แสดงในหน้า `/my/trials/new` เตือนว่า T3 เสี่ยงกว่าแขนงอื่นเพราะตัด Haiter ออกทั้งขั้น
  ควรทำหลัง T1/T2 ให้ผลแล้ว ตามที่ new_idea.md แนะนำ
- **ปัญหาที่ต้องแก้เพิ่มจากการเพิ่ม T3**: เนื้อหาขั้น "ฟอกฆ่าเชื้อ" ในคู่มือใช้ร่วมกันทุกแขนง (ตามการตัดสินใจเดิมว่า
  ไม่แยกเนื้อหาต่อแขนง) ซึ่งเขียนไว้สำหรับ Haiter ล้วน ถ้าไม่มีอะไรบอกผู้ใช้ที่ทำ T3 จะเห็นคำสั่งเจือจาง Haiter
  ทั้งที่แขนงนี้ไม่ได้ใช้ Haiter เลย — นี่คือปัญหาเดียวกับที่แบนเนอร์ (ที่ deferred ไว้รอบก่อน) ถูกออกแบบมาแก้อยู่แล้ว
  จึงทำสองอย่างนี้พร้อมกันแทนที่จะแยก
- เพิ่ม `RoundView.trialArmRole` / `trialArmLabel` / `sterilization` ใน `round-adapter.ts` (`buildRoundView`)
  เพื่อส่งข้อมูลระดับ lot ที่ step-runner.tsx ต้องใช้ตัดสินใจแสดงแบนเนอร์ ลงมาถึง UI โดยไม่ต้องส่ง `ExperimentLot`
  ทั้งก้อนเข้าไปเพิ่ม (มีแค่ field ที่ใช้จริง)
- คอมโพเนนต์ใหม่ `src/components/rounds/sterilization-method-banner.tsx` แสดงเฉพาะขั้น `sterilize`
  (`step-runner.tsx` เช็ค `step.id === "sterilize"`) และเฉพาะรอบที่มี `sterilization` (คือแขนงของชุดทดลองเท่านั้น
  รอบทั่วไปไม่เห็นอะไร): ถ้า method เป็น `nadcc-soak` แสดงข้อความสีแดง (`--pl-stop`) บอกให้ข้ามคำสั่ง Haiter ด้านล่าง
  ทั้งหมดแล้วทำ NaDCC soak แทน ถ้ามี `rinseWater` (T1/T2) แสดงข้อความสีเทา (`--pl-sunk`) บอกว่าต้องล้างเพิ่มอีก 3
  รอบด้วยน้ำ rinse ตามวิธีที่กำหนด ทั้งสองแบบใช้ `RichText` เชื่อมคำว่า NaDCC ไปหาคลังสาร
- Rename `trial-overview.ts` field `rinseMethodLabel` → `methodLabel` เพราะตอนนี้ต้องอธิบายทั้งวิธีฆ่าเชื้อหลัก
  (Haiter vs NaDCC soak) ไม่ใช่แค่ rinse เสริม — T3 ไม่มี rinseWater เลยแต่เปลี่ยนวิธีหลัก ถ้าไม่เช็ค `method` ก่อน
  จะ fallback ไปข้อความ "ค่าเริ่มต้นของระบบ" ที่ผิด (เหมือน Control-A) ทั้งที่ความจริงคือใช้ NaDCC ล้วนแทน
- ทดสอบจริงผ่านเบราว์เซอร์ (demo mode): เปิดชุดทดลองห้าแขนงสำเร็จ → หน้าภาพรวมแสดง T3 ถูก label →
  เปิดขั้นฟอกฆ่าเชื้อของ T3 เห็นแบนเนอร์แดง "แขนงนี้ข้ามขั้น Haiter ทั้งขั้น" พร้อมลิงก์ NaDCC กดดูได้ →
  เปิดขั้นเดียวกันของ T2 เห็นแบนเนอร์เทา "แขนงนี้ใช้น้ำ rinse เพิ่มเติมหลังฟอก" → เปิดของ Control-A ไม่เห็นแบนเนอร์เลย
  (ตามที่ออกแบบไว้ว่ารอบทั่วไปไม่ควรเห็นอะไรเพิ่ม)
- Verification รอบนี้:
  - `npm test`: ผ่าน 560 tests (เพิ่มจาก 551 เดิม 9 เทสใหม่), skip 10
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน

## 2026-08-09 — สร้างระบบชุดทดลองหลายแขนง (multi-arm trial) เริ่มจากแบบ NaDCC เทียบ Haiter ใน new_idea.md หัวข้อ 15

- ต่อจากรอบก่อนหน้าในวันเดียวกัน (เชื่อมคลังสาร) ผู้ใช้ขอให้เอา new_idea.md ไปทำต่อเป็น protocol จริงในระบบ
- ระบบเดิมมีแค่ "bracket testing" ซึ่งทดสอบตัวแปรเดียว (ความเข้มข้น) 3 ระดับภายใน lot เดียวกัน ไม่รองรับการเทียบ
  วิธีต่างกัน (NaClO vs NaDCC) ข้าม lot หรือกระปุกควบคุมที่ไม่มี explant เลย จึงต้องออกแบบใหม่
- **พบ dead code สำคัญระหว่างสำรวจ**: `rinse-water-planning.ts` มีฟังก์ชันเตรียมน้ำ rinse คลอรีนต่ำอยู่แล้วตั้งแต่ก่อนหน้านี้
  แต่ไม่มีจุดใช้งานจริงที่ไหนเลย และตั้งค่าความเข้มข้นไว้ที่ 0.003% (30 ppm) ซึ่งผิดหลักสิบเท่าจากที่ new_idea.md
  คำนวณละเอียดแล้วสรุปที่ 0.03% (300 ppm) ผู้ใช้ยืนยันให้แก้เป็น 300 ppm ตามการคำนวณใหม่
- **การตัดสินใจด้านสถาปัตยกรรม**: ไม่สร้าง Firestore entity ใหม่สำหรับ "ชุดทดลอง" เพิ่ม `trialId`/`armRole`/`armLabel`/
  `isBlank` เป็น field เสริมบน `ExperimentLot` แทน กลุ่มของชุดทดลองคือ lot ที่ `trialId` ตรงกันเท่านั้น ไม่มีแหล่ง
  ความจริงที่สองให้ desync เพราะ `createLot` เดิมรับ `CreateLotInput` เต็มรูปแบบอยู่แล้ว ไม่ต้องเพิ่ม repository method ใหม่
- **สำคัญ**: `normalizeExperimentLot` (ทำงานทุกครั้งที่ Firestore อ่าน lot กลับมา) whitelist field ที่ผ่านได้ทีละตัว
  ถ้าไม่เพิ่ม field ใหม่เข้าไปตรงนี้ด้วย ข้อมูลแขนงของชุดทดลองจะหายเงียบ ๆ ทุกครั้งที่โหลดหน้าเว็บจริง (ใช้ได้แค่ตอน
  ทดสอบด้วย memory repository เท่านั้น) แก้จุดนี้แล้วพร้อมเทสยืนยัน
- Control-B (กระปุกเปล่า ไม่มี explant) ไม่มี entity ใหม่ ใช้ `Observation` stream เดิมของ lot ปกติ เพิ่ม field
  `isBlank` บน lot แล้วให้ `buildRoundView` (round-adapter.ts) กรองขั้น `select-explant` และ `cut` ออกเมื่อ isBlank
  เป็น true ขั้นฟอกฆ่าเชื้อ/ลงอาหาร/ตรวจปนเปื้อนยังคงอยู่ครบเพราะกระปุกเปล่าก็ต้องผ่านขั้นเหล่านี้เหมือนกัน
- T1/T2 (NaClO vs NaDCC rinse) ใช้ `RinseWaterMethod` เดิมที่มีอยู่แล้วแต่ไม่เคยถูกใช้จริง เพิ่มค่า `"nadcc"` และ
  ฟังก์ชัน `buildNaDccRinseWaterSnapshot` คู่กับของเดิม ทั้งคู่ตั้งเป้า 300 ppm เท่ากัน ฝังไว้ใน `sterilization.rinseWater`
  ตอนสร้าง lot โดยตรง ไม่ต้องมี UI เลือกวิธีแยกต่างหาก เพราะแม่แบบชุดทดลองล็อกไว้ต่อแขนงอยู่แล้ว
- ไฟล์ใหม่: `src/lib/trials/nadcc-vs-haiter-trial.ts` (แม่แบบ 4 แขนง Control-A/B, T1, T2 ล็อกตายตัวตามหัวข้อ 15
  ไม่ใช่ trial-builder ทั่วไป) และ `src/lib/trials/trial-overview.ts` (view-model เปรียบเทียบ ไม่คำนวณสถิติใหม่
  แค่เรียงและดึงบันทึกล่าสุดมาแสดง พร้อมข้อความเตือนแบบเดียวกับ bracket testing ว่าไม่ใช่ข้อพิสูจน์ทางสถิติ)
- หน้าใหม่: `/my/trials/new` (เลือกเริ่ม เห็นรายละเอียดสี่แขนงก่อนกด ไม่ใช่สร้างอัตโนมัติแบบ `/my/rounds/new`) และ
  `/my/trials/[trialId]` (การ์ดเทียบทั้งสี่แขนง เชื่อมกลับไปหน้า `/my/rounds/[lotId]` เดิมเพื่อบันทึกผลรายวัน)
  เพิ่มลิงก์เข้าถึงจากหน้า `/my/rounds`
- T3 (NaDCC ล้วนแทน Haiter ทั้งขั้น 24-48 ชม.) ยังไม่ทำตามที่ตกลงไว้ ต้องเพิ่ม `SterilizationMethod` ใหม่เพราะ
  T3 แทนที่ขั้น Haiter ทั้งขั้น ไม่ใช่แค่เปลี่ยนน้ำ rinse หลังขั้นนั้น แต่ไม่ต้อง rework ระบบกลุ่ม/repository/skip-logic
  ที่ทำไว้แล้ว เพิ่มแค่ arm ใหม่ในแม่แบบได้เลย
- ยังไม่ทำ (cosmetic, deferred ตามแผน): แบนเนอร์แสดงวิธี rinse ที่เลือกไว้ตรงขั้นฟอกฆ่าเชื้อใน step-runner.tsx —
  ตอนนี้ผู้ใช้ต้องเปิดหน้าภาพรวมชุดทดลองเพื่อดูว่าแขนงไหนใช้วิธีไหน ไม่ได้เห็นซ้ำตรงหน้าขั้นตอน
- ทดสอบจริงผ่านเบราว์เซอร์ (demo mode): เปิด `/my/trials/new` → กดเริ่ม → redirect ไปหน้าภาพรวมพร้อมสี่การ์ดเรียง
  ลำดับถูกต้องและ label วิธี rinse ถูกต้อง → เปิด Control-B พบ 13 ขั้น (ข้าม `select-explant`/`cut`) → เปิด T2
  พบครบ 15 ขั้น ยืนยัน skip-logic ทำงานถูกต้องเฉพาะแขนงที่ isBlank
- Verification รอบนี้:
  - `npm test`: ผ่าน 551 tests (เพิ่มจาก 537 เดิม 14 เทสใหม่), skip 10
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน ทุก route รวม `/my/trials/new` และ `/my/trials/[trialId]`
...
## 2026-07-27 — อธิบายและคำนวณ MS 1×

- `1×` หมายถึงความเข้มข้น MS เต็มสูตรตามอัตราของผู้ผลิต ไม่ใช่ 1 กรัม
- ห้าม hard-code กรัมของ MS เพราะผลิตภัณฑ์อาจมีสูตร/ส่วนประกอบและอัตรา g/L ต่างกัน
- ตารางสูตรทุกปริมาตรแสดงข้อความ `เต็มสูตร ไม่ใช่ 1 g; คำนวณจาก g/L บนฉลาก`
- เครื่องคำนวณสูตรในคลังความรู้มี input `อัตรา MS basal salts บนฉลาก (g/L)` และคำนวณกรัมตามปริมาตรรวม
- ขั้นเตรียมอาหารใน Lot มีเครื่องคำนวณ MS ในหน้าเดียว:
  - ผู้ใช้กรอก g/L จากฉลาก
  - ระบบคำนวณกรัมจาก `อัตราฉลาก × ตัวคูณสูตร × ปริมาตร mL ÷ 1000`
  - ถ้ายังไม่กรอก ระบบหยุดและเตือนว่าห้ามใช้ค่าของผลิตภัณฑ์อื่นแทน
- Verification:
  - targeted tests: ผ่าน 10 tests
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน
  - `npm run firebase:verify`: ผ่าน 260 tests
  - `npm run ui:verify`: ผ่าน 14 viewport ตั้งแต่ 360 ถึง 1920px

## 2026-07-31 — Protocol v2 แบบทีละขั้นสำหรับ Pink Princess + Haiter

- เปลี่ยนเส้นทาง Lot ใหม่ของ Pink Princess + Haiter เป็น Protocol v2 จำนวน 14 ขั้นหลัก:
  1. ตรวจต้นแม่และสุขภาพ
  2. หาและทำเครื่องหมายข้อกับตาข้าง
  3. กำหนดจำนวนชิ้นพืชและกระปุก
  4. เตรียมน้ำ น้ำยา และสารละลายตั้งต้น
  5. ฆ่าเชื้อกระปุกและฝา
  6. เตรียมอาหาร
  7. เติม Haiter ลงในอาหาร
  8. รอตรวจ Blank 48 ชั่วโมง
  9. ล้างต้นแม่และเตรียมชิ้นส่วน
  10. เตรียมกล่องปลอดเชื้อและเครื่องมือ
  11. ตัดชิ้นพืชก่อนฟอก
  12. ฟอกผิวชิ้นพืชและล้าง
  13. ตัดแต่ง ลงอาหาร และปิดภาชนะ
  14. ตรวจการปนเปื้อนและการตั้งตัว
- หน้า Runner แสดงงานปัจจุบันเพียงขั้นเดียว รายการ 14 ขั้นถูกย่อไว้ก่อน ขั้นถัดไปล็อกจนกว่าขั้นก่อนหน้าจะเสร็จ
- เอาระบบบันทึกรูปและผลแบบ radio ออกจาก Protocol v2 เหลือเฉพาะ checklist, ค่าที่ต้องวัด, Timer, `ฉันพบปัญหา` และ `ทำขั้นนี้เสร็จแล้ว`
- Timer บันทึกเวลาเริ่ม/สิ้นสุดใน Step Run จึงปิดหน้าและกลับมาใหม่ได้โดยไม่รีเซ็ต
- ขั้นเตรียมอาหารแสดงสูตรของ Lot และเครื่องคำนวณ MS/stock ในหน้าเดียว
- ขั้นเติม Haiter ใช้ค่าจาก Lot และสร้างคำสั่งตวงจริง รวม working dilution เมื่ออุปกรณ์ตวงปริมาตรตรงไม่ได้
- ขั้นฟอกชิ้นพืชแปลงฉลาก Haiter เป็นคำสั่งผสมจริง เช่นฉลาก 6% จะสั่ง `Haiter 10.00 mL + น้ำปลอดเชื้อ 90.00 mL` เพื่อเตรียม active chlorine 0.6% ปริมาตร 100 mL
- ขั้นฆ่าเชื้อกระปุก/ฝาอ้างอิง `source-csup-2012` และ `source-naocl-vessels-2009`; ขั้นที่ยังประยุกต์หรือทดลองไม่ถูกยกระดับเป็น Verified
- Wizard แก้กรณีเริ่มจากชื่อ Pink Princess โดยไม่มี Plant record ให้ผูก `cultivar-pink-princess` อัตโนมัติ จึงไม่ตกเป็น Lot รุ่นเก่า
- Lot รุ่นเดิมเก็บไว้ดูประวัติแบบอ่านอย่างเดียวและไม่ย้ายสถานะเก่าเข้า 14 ขั้นใหม่โดยการเดา
- Firestore Step Run ตัด optional field ที่เป็น `undefined` ก่อนบันทึก ป้องกัน `Unsupported field value: undefined`
- อัปเดต browser verification ให้สร้าง Pink Princess Lot v2 จริง ตรวจ 14 ขั้น, การล็อกขั้น, problem flow, completion flow, scroll-to-top, no-photo และ no-radio
- Verification รอบส่งงาน:
  - `npm test`: ผ่าน 264 tests, skip 10 เมื่อไม่เปิด emulator
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน
...
## 2026-07-31 — บันทึกขั้นที่ทำไปแล้วโดยไม่เริ่มใหม่

- เพิ่มปุ่ม `ฉันทำขั้นนี้ไว้แล้ว` ในทุกขั้นของ Protocol v2 เพื่อให้ Lot เดิมเดินหน้าต่อได้โดยไม่ต้องทำงานซ้ำ
- ผู้ใช้กรอกวันที่/เวลาที่เริ่มจริงและบันทึกสั้น ๆ; ขั้นที่ไม่มี Timer ให้กรอกเวลาที่ทำเสร็จจริงด้วย
- ขั้นที่มี Timer คำนวณเวลาสิ้นสุดจากเวลาเริ่มจริง:
  - ถ้าครบเวลาแล้ว ระบบบันทึกผ่านทันทีเมื่อ checklist และค่าบังคับครบ
  - ถ้ายังไม่ครบ ระบบบันทึกสถานะรอและนับเฉพาะเวลาที่เหลือ ไม่เริ่ม Timer ใหม่
- ปฏิเสธเวลาเริ่มในอนาคต เวลาจบในอนาคต และเวลาจบที่อยู่ก่อนเวลาเริ่ม
- การบันทึกย้อนหลังไม่บังคับรูป และไม่สร้าง Lot ใหม่
- Step Run เพิ่ม `completionMode: "retrospective"` และ `retrospectiveRecordedAt`; Audit History แสดง `บันทึกย้อนหลังว่าขั้นนี้เสร็จแล้ว` หรือ `บันทึกเวลาเริ่มย้อนหลัง`
- การทำสดยังถูกบันทึกเป็น `completionMode: "live"` จึงแยกจากข้อมูลย้อนหลังได้
- Browser sandbox สร้าง Pink Princess + Haiter Lot จริง เปิดแบบบันทึกย้อนหลัง กรอกเวลา/บันทึก ยืนยันผล และตรวจว่าขั้นถัดไปปลดล็อก
- Verification:
  - targeted tests: ผ่าน 12 tests
  - `npm test`: ผ่าน 271 tests และ skip 10 เมื่อไม่เปิด emulator
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน
...
- ผู้ใช้เลือกเพียงว่า `ฉันจะเริ่มทำต่อจาก` ขั้นใด ระบบจะระบุขั้นก่อนหน้าที่ยังไม่มีผลว่า `ทำไปแล้ว` ในคำสั่งเดียว
- Catch-up flow ไม่ขอ note รูป เวลาเริ่ม หรือเวลาจบแยกทุกขั้น; วันที่โดยประมาณเป็นข้อมูลเสริมและไม่บังคับ
- ถ้าช่วงที่ข้ามมี Timer ระบบถามยืนยัน `ฉันยืนยันว่าครบเวลาที่กำหนดแล้ว` เพียงครั้งเดียว และไม่เริ่มจับเวลาใหม่
- ถ้าไม่ยืนยัน Timer ระบบไม่ปิดขั้นดังกล่าว ผู้ใช้ต้องเริ่มต่อที่ Timer นั้น
- ระบบไม่เขียนทับผลเดิมที่เป็น `Passed`; ถ้าพบ `Needs review` หรือ `Failed` จะหยุดและให้กลับไปจัดการขั้นนั้นก่อน
- การบันทึกหลายขั้นใช้ Firestore batch เดียว จึงสำเร็จทั้งหมดหรือไม่บันทึกทั้งหมด พร้อม audit action `carried_forward`
- Audit History แสดง `ระบุว่าขั้นนี้ทำไปแล้ว` และเก็บ metadata โดยไม่สร้างเวลาสิ้นสุดปลอม
- แบบบันทึกย้อนหลังรายขั้นถูกลดเหลือ:
  - ขั้นทั่วไป: วันที่โดยประมาณซึ่งไม่บังคับ
  - ขั้น Timer: checkbox ยืนยันว่าครบเวลาแล้ว และวันที่โดยประมาณซึ่งไม่บังคับ
  - ไม่บังคับเขียน note หรือแนบรูป
- หลังยืนยัน ระบบเปิดขั้นที่เลือกและเลื่อนกลับหัวขั้นทันที; เมื่อ reload จะกลับมาที่ขั้นแรกที่ยังไม่ได้ทำ ไม่ย้อนกลับขั้น 1
- Browser sandbox สร้าง Pink Princess + Haiter Lot จริง เลือกเริ่มต่อที่ขั้น 9 ยืนยัน Blank timer ครั้งเดียว ตรวจว่าขั้น 1–8 ผ่าน เปิดขั้น 9 และ reload แล้วยังคงเริ่มที่ขั้น 9
- Verification รอบส่งงาน:
  - `npm test`: ผ่าน 283 tests และ skip 10 เมื่อไม่เปิด emulator
  - `npm run lint`: ผ่าน
  - `npm run build`: ผ่าน
  - `npm run firebase:verify`: ผ่าน 293 tests ทั้งหมด
  - `npm run ui:verify`: ผ่าน 14 viewport ได้แก่ 360, 375, 390, 412, 428, 600, 744, 768, 820, 834, 1024, 1280, 1440 และ 1920px
- Implementation commits: `25cd12f`, `422af6f`, `d3d8157`, `77adbc8`, `87e1999`
- push เข้า `master` แล้ว
- Vercel Production deployment `dpl_3Wzhuon8R8WJEoDjWvzrb4YKhU1A` ขึ้น `READY`
- Production URL ตอบกลับ HTTP 200: `https://tissue-experiment-93.vercel.app/`

## 2026-08-02 เฟส 2 คู่มือสาธารณะ

ตรวจด้วย `npm run ui:verify` ที่ `UI_BASE_URL=http://localhost:3100` บน production build

- ผ่านครบ 14 ความกว้าง: 360, 375, 390, 412, 428, 600, 744, 768, 820, 834, 1024, 1280, 1440, 1920 px
- เพิ่มการตรวจหน้าสาธารณะเข้าไปใน script คือ `/`, `/guide/pink-princess` และ `/guide/pink-princess/step/7`
- แก้ `enterDemo` และ `returnToApp` ให้เข้าแอปที่ `/my` เพราะ `/` กลายเป็นหน้าคู่มือสาธารณะแล้ว

ตรวจด้วยตาในเบราว์เซอร์ทั้งโหมดสว่างและโหมดมืด

- ปุ่มสลับธีมทำงาน ป้ายบนปุ่มเปลี่ยนตามโหมดจริง และค่าที่เลือกถูกจำไว้ข้ามหน้า
- ภาพประกอบเปลี่ยนสีตามโหมด ไม่ค้างเป็นชุดสีของโหมดสว่าง
- ขอบหนาและเงาทึบยังอ่านออกในโหมดมืด

ข้อบกพร่องที่พบและแก้ไปแล้วในรอบนี้

- ป้ายกำกับเล็กเดิมใช้ `letter-spacing` กับ `text-transform: uppercase` ทำให้ตัวอักษรไทยถ่างและอ่านยาก แก้โดยถอดทั้งสองอย่างออกจาก `.pl-mono`
- สีแดงของป้าย "ยังไม่มีงานรองรับ" ในโหมดมืดคอนทราสต์ต่ำเกินไปเมื่อวางตัวหนังสือสีเข้มทับ ปรับ `--pl-red` ของโหมดมืดให้สว่างขึ้นเป็น `#e08b76`
- ถอดฟอนต์ `IBM_Plex_Mono` ออกหลังเลิกใช้ เพื่อไม่ให้โหลดไฟล์ฟอนต์ที่ไม่ได้ใช้ทุกหน้า

ข้อสังเกตเรื่องความไม่เสถียรของ script

`verifyWizard` timeout ที่ 30 วินาทีและล้มเหลวสองครั้งระหว่างที่เครื่องรัน build พร้อมกัน แต่ผ่านทั้งก่อนหน้าและหลังจากนั้นเมื่อเครื่องว่าง เป็นความไม่เสถียรจากภาระเครื่อง ไม่ใช่ regression ของเฟสนี้ ถ้าเจอซ้ำในอนาคตควรเพิ่ม timeout ของขั้นนี้

## 2026-08-02 เฟส 3a รอบเพาะและตัวเดินขั้นตอน

เดินทั้งเส้นทางด้วยมือบน production build ที่ `http://localhost:3100`

- เปิด `/guide/pink-princess` กดปุ่มเริ่มรอบ ระบบพาไป `/my/rounds/new?slug=pink-princess` แล้วติดหน้าล็อกอินถูกต้อง
- เข้าโหมดสาธิต ระบบสร้างรอบและ redirect ไปหน้ารอบอัตโนมัติ ขั้นที่ 1 ถูกไฮไลต์ว่าทำต่อตรงนี้
- เข้าขั้นที่ 7 กรอกคลอรีนออกฤทธิ์ 1 เปอร์เซ็นต์ เวลา 12 นาที ล้าง 3 รอบ พร้อมบันทึกข้อความ แล้วกดผ่าน
- กลับไปหน้ารอบ ความคืบหน้าขึ้นเป็นผ่านแล้ว 1 จาก 14 ขั้น และขั้นที่ 7 แสดงค่าที่บันทึกไว้ครบ

ทดสอบออฟไลน์

- ปลอมสถานะออฟไลน์ในเบราว์เซอร์ แถบเตือนขึ้นทันทีทั้งหน้ารอบและหน้าขั้นตอน
- กดบันทึกตอนออฟไลน์ ได้ข้อความยืนยันว่าบันทึกว่าผ่านแล้ว และข้อความที่พิมพ์ไว้ยังอยู่ครบ ไม่หาย
- แถบเตือนบอกด้วยว่าช่วงออฟไลน์แนบรูปไม่ได้ ให้ถ่ายเก็บไว้ก่อน

ข้อบกพร่องที่พบจากการดูของจริงและแก้แล้ว

- หน้ารอบเคยแสดงค่าที่วัดด้วย id ภายในเป็นภาษาอังกฤษ เช่น `sterilize-minutes 12` แก้เป็นชื่อที่ผู้ใช้อ่านออกพร้อมหน่วย คือ `เวลาฟอกที่ใช้จริง 12 min` และเพิ่มเทสต์กันไม่ให้ id หลุดออกมาอีก
- `auth-gate` และ `lab-shell` ยังเขียนว่า Philodendron Lab แก้เป็น Plantlover Lab แล้ว

ยังไม่มีในเฟสนี้ คือการแนบรูปในขั้นตอน และการลบ runner เดิมทั้ง 4 ตัว อยู่ในแผนถัดไป

## 2026-08-03 เฟส 3b หลักฐานภาพรายขั้น

สิ่งที่ค้นพบก่อนเริ่ม

ระบบเดิม **ไม่เคยต่อการแนบรูปรายขั้นให้เสร็จ** `guided-protocol-runner.tsx` อ่าน `evidenceObservationId`
เพื่อตัดสินว่าจะเปิดพื้นที่อัปโหลดไหม แต่ไล่ทั้ง repo แล้วไม่มีที่ไหนเขียนค่านั้น ผู้ใช้จึงเห็นข้อความ
ให้กดบันทึกร่างก่อนค้างอยู่ตลอด งานนี้จึงเป็นการสร้างใหม่ ไม่ใช่ย้ายของเดิม

ที่ทดสอบแล้วบน production build

- โหมดสาธิต หน้าขั้นตอนแสดงหัวข้อหลักฐานภาพ บอกว่ายังไม่มีรูปของขั้นนี้ และอธิบายว่า
  โหมดสาธิตแนบรูปไม่ได้เพราะต้องยืนยันตัวตนกับเซิร์ฟเวอร์ ไม่มีช่องเลือกไฟล์ปรากฏ
- สลับเป็นออฟไลน์ เหตุผลเปลี่ยนเป็นเรื่องเน็ตหลุดทันที และแถบเตือนออฟไลน์ขึ้นด้วย
- `npm test` 413 ผ่าน 10 skip, `npm run lint` และ `npm run build` ผ่าน

**ที่ยังไม่ได้ทดสอบและต้องยืนยันบน deployment ที่ตั้งค่าแล้ว**

การอัปโหลดจริงยังไม่ได้ลอง เพราะต้องมี Firebase Auth ผู้ใช้จริงกับ Cloudinary credentials
ซึ่งเครื่องที่พัฒนาไม่มี สิ่งที่ต้องยืนยันคือ ขอลายเซ็นจาก `/api/media/sign` สำเร็จ อัปโหลดขึ้น Cloudinary ได้
รูปกลับมาแสดงในหน้าขั้นตอน และ `mediaIds` ถูกบันทึกลง step run **ห้ามถือว่าผ่านจนกว่าจะลองจริง**

ที่ยังไม่ทำในเฟสนี้

ไม่ได้ลบ runner เดิมหรือเส้นทางเก่า เพราะไล่สายพึ่งพาแล้วพบว่า `philodendron-knowledge.ts` มีของพึ่งพา 9 จุด
รวมถึง `firestore-protocol-repository.ts` ที่ตกลงไว้ว่าจะเก็บ และหน้า `/knowledge` ยังเป็นที่อยู่ของ
เครื่องคำนวณสูตรอาหารที่คู่มือใหม่อ้างถึงในขั้นทำอาหาร การลบต้องรอเฟส 4 สร้างของทดแทนก่อน

## 2026-08-03 เฟส 4a เครื่องคำนวณสูตรอาหารสำหรับงานเล็ก

ทดสอบบน production build ที่หน้า `/guide/pink-princess/step/6` เลือกสูตรระยะเพิ่มจำนวนยอด

- 6 กระปุก คูณ 25 มล. เท่ากับ 150 เผื่อสูญเสีย 15 เปอร์เซ็นต์ ได้ **173 มิลลิลิตร** ตรงกับที่คำนวณด้วยมือ
- MS basal salts 0.766 g, Sucrose 5.19 g, Agar 1.297 g ทั้งสามตัวชั่งได้
- BAP ขึ้นว่า **ชั่งไม่ได้** เพราะสูตรต้องการ 0.173 มิลลิกรัม ต่ำกว่าเครื่องชั่งที่อ่านได้ 10 มิลลิกรัม
  แล้วแสดงขั้นตอนทำน้ำยาแม่ให้ครบ จบที่ตวง 1.73 มล. ซึ่งตวงได้ด้วยไซริงค์ทั่วไป

ข้อบกพร่องที่พบจากการดูผลลัพธ์จริงและแก้แล้ว

คำสั่งเดิมเริ่มที่ `ตวง stock เดิม 5 mL` **โดยไม่เคยบอกว่าน้ำยาแม่ 1 mg/mL นั้นได้มาจากไหน**
ผู้ใช้ที่บ้านยังไม่มีของชิ้นนั้น เพิ่มขั้นแรกให้บอกว่าชั่งผง 50 มิลลิกรัม ละลายน้ำเป็น 50 มิลลิลิตรก่อน
มวลที่สั่งให้ชั่งตั้งไว้สูงกว่าขีดต่ำสุดของเครื่องชั่งห้าเท่า ไม่ใช่พอดีเป๊ะ เพราะการชั่งที่ขีดต่ำสุด
มีความคลาดเคลื่อนสูง มีเทสต์บังคับระยะเผื่อนี้ไว้แล้ว

เครื่องคำนวณอยู่ในขั้นทำอาหารทั้งฝั่งคู่มือที่อ่านอย่างเดียวและฝั่งตัวเดินขั้นตอน
ปิดช่องว่างที่คู่มือเคยเขียนว่า `ใช้เครื่องคำนวณในระบบ` แต่ยังไม่มีเครื่องคำนวณในเส้นทางใหม่

ยังไม่ทำในเฟสนี้ คือระบบเลือกเส้นทางอุปกรณ์ตามของที่มี ซึ่งเป็นอีกครึ่งของเฟส 4
ความละเอียดของเครื่องชั่งและอุปกรณ์ตวงจึงยังเป็นช่องกรอกในหน้าเครื่องคำนวณ ยังไม่ได้จำไว้ให้

## 2026-08-03 เฟส 4b เส้นทางอุปกรณ์

คู่มือบอกว่าต้องได้อะไร ระบบบอกว่าด้วยของที่มีจะได้มายังไง มีสี่สิ่งที่ต้องได้คือ
อาหารปลอดเชื้อ น้ำปลอดเชื้อ ภาชนะปลอดเชื้อ และผิวชิ้นพืชสะอาด

ทดสอบบน production build ที่ `/my/equipment`

- ยังไม่ติ๊กอะไร ทุกอย่างตัน และระดับรวมขึ้นว่ายังสรุปไม่ได้ ถูกต้อง
- ติ๊กไฮเตอร์อย่างเดียว ได้อาหารด้วยวิธีเติมไฮเตอร์ลงในอาหาร และได้การฟอกผิว
  แต่ **น้ำปลอดเชื้อยังตัน** ซึ่งตรงกับเคสที่ตั้งโจทย์ไว้ตั้งแต่ต้น และระบบกางทางเลือกให้เลือกเอง
  โดยไม่เลือกให้อัตโนมัติ
- เพิ่มน้ำเกลือปลอดเชื้อจากร้านขายยา เส้นทางครบและระดับรวมขึ้นเป็นประยุกต์
- เพิ่มเตาต้มน้ำเข้าไปด้วย ระดับรวมยังเป็นประยุกต์ เพราะระบบเลือกวิธีที่หลักฐานดีที่สุด
  ที่ทำได้ ไม่ได้หยิบวิธีที่อ่อนกว่ามาใช้

บั๊กที่พบจากการทดสอบจริงและแก้แล้ว สองข้อ

1. เครื่องคำนวณไม่รับค่าจากชุดอุปกรณ์ที่บันทึกไว้ เพราะ `useState` อ่านค่าแค่ตอน mount
   แต่ชุดอุปกรณ์โหลดมาแบบ async ทีหลัง แก้โดยใส่ `key` ให้เครื่องคำนวณเริ่มใหม่เมื่อค่าโหลดมาถึง
2. repository ของอุปกรณ์ในโหมดสาธิตเก็บในหน่วยความจำล้วน ต่างจาก repository ตัวอื่น
   ที่เขียนผ่าน demo storage ค่าจึงหายทุกครั้งที่โหลดหน้าใหม่ แก้ให้ใช้ demo storage เหมือนกัน

ยืนยันหลังแก้แล้วว่า ตั้งเครื่องชั่งเป็น 1 มิลลิกรัม บันทึก แล้วเปิดขั้นทำอาหารของรอบใหม่
เครื่องคำนวณรับค่า 1 มาจริง และ BAP เปลี่ยนจากชั่งไม่ได้เป็นชั่งได้ตามที่ควรเป็น

ยังไม่ทำ คือการล็อกเส้นทางที่เลือกไว้กับรอบเพาะ ณ เวลาเริ่มรอบ และการบังคับกระปุกเปล่าคุม
เมื่อเส้นทางเป็นยังไม่มีงานรองรับ ทั้งสองอย่างต้องรอให้เส้นทางถูกผูกกับรอบก่อน

## 2026-08-03 รอบเก่าเข้าถึงได้ และเริ่มลบของเก่า

พบว่าหน้า `/my/rounds` กรองรอบที่ `protocolId` resolve เป็น slug ของคู่มือไม่ได้ทิ้งไปเงียบ ๆ
ซึ่งทำให้ล็อตที่สร้างก่อนระบบคู่มือใหม่ **เข้าถึงไม่ได้เลย** ไม่ใช่แค่ดูไม่สวย
แก้ให้แสดงเป็นหัวข้อแยกพร้อมลิงก์ไปหน้าเดิม

ทดสอบบน production build ในโหมดสาธิต เห็นรอบเก่าสามรอบกลับมาแล้ว คือ Violin variegated,
Philodendron green control และ Pink Princess ทั้งหมดลิงก์ไป `/experiments/<id>` ได้

ลบโค้ดตายไป 726 บรรทัด คือ `guided-protocol-runner` ที่ไม่มีใครนำเข้าเลย
กับ `beginner-step-guide` ที่มีแต่ตัวแรกใช้ และ `protocol-reference-visual` ที่ไม่มีใครใช้
เทสต์ลดจาก 446 เหลือ 434 เพราะเทสต์ของไฟล์ที่ลบหายไปด้วย ไม่ใช่เพราะมีอะไรพัง

runner อีกสามตัวยังอยู่ เพราะหน้า `/experiments` ยังต้องใช้ และหน้านั้นคือสิ่งที่ทำให้
ล็อตเก่าอ่านได้ ต้องมีทางอ่านล็อตเก่าในระบบใหม่ก่อนจึงจะลบได้

## 2026-08-03 ลบเส้นทางเดิมและ runner ที่เหลือ

ทำหน้าอ่านล็อตเก่าในดีไซน์ใหม่ที่ `/my/rounds/legacy/<id>` แสดงหัวรอบ บันทึกการสังเกต
และบันทึกรายขั้นที่เคยทำ แบบอ่านอย่างเดียว พร้อมบอกว่าถ้าจะทำต่อให้เริ่มรอบใหม่จากคู่มือ

เมื่อล็อตเก่าอ่านได้ในระบบใหม่แล้ว จึงลบของเดิมได้ **รวม 2,716 บรรทัด**

- `src/app/experiments` ทั้งโฟลเดอร์
- `src/app/protocols` ทั้งโฟลเดอร์
- `src/components/protocols` ทั้งโฟลเดอร์ รวม runner ที่เหลือทั้งสามตัว
- `src/components/experiments` ทั้งโฟลเดอร์
- เมนู Experiments และ Protocols ออกจาก `lab-shell`

ปิดปัญหา runner สี่ตัวซ้อนกันที่วิเคราะห์ไว้ตั้งแต่วันแรกได้สมบูรณ์ เหลือตัวเดินขั้นตอนตัวเดียว

ตรวจบน production build แล้ว เส้นทางใหม่ทั้งหมดคืน 200 ส่วน `/experiments` และ `/protocols`
คืน 404 ตามที่ตั้งใจ และเปิดล็อตเก่า VIO-001 อ่านได้จริงในหน้าใหม่

เทสต์ลดจาก 439 เหลือ 407 เพราะเทสต์ของไฟล์ที่ลบหายไปด้วย ไม่ใช่เพราะมีอะไรพัง
ต้องแก้ `lab-shell.test.tsx` ที่ยังคาดหวังเมนูเดิมสองรายการ

ยังเหลือ `protocol-templates.ts` ที่ `plant-experiment-summary` และ
`firestore-protocol-repository` ยังพึ่งอยู่ ซึ่งผูกกับหน้า `/plants` ที่ยังไม่มีของทดแทน

## 2026-08-03 เลิกใช้บันทึกต้นแม่แยก

ผู้ใช้ยืนยันว่าส่วนใหญ่ทำรอบเดียวจบต่อต้น จึงไม่ต้องมีตัวตนต้นแม่แยกจากรอบ
และที่สำคัญกว่านั้น **ขั้นที่ 1 กับ 3 ของคู่มือเก็บข้อมูลต้นแม่อยู่แล้ว** คือถ่ายรูปตั้งต้น
และยืนยันชนิดพร้อมระดับความมั่นใจ ข้อมูลชุดเดียวกับที่ `PlantRecord` เคยเก็บ
แต่ถามในจังหวะที่ผู้ใช้กำลังทำจริง ไม่ใช่เป็นด่านก่อนเริ่ม

ลบไป `src/app/plants`, `src/components/plants`, `plant-profile`, และ repository ของ plant
ทั้ง memory, firestore และ factory พร้อมเมนู Plants ออกจาก `lab-shell`

**ผลข้างเคียงที่ต้องรู้** ผู้ใช้ที่เคยสร้าง Plant Record ไว้จะเข้าถึงข้อมูลนั้นไม่ได้อีก
ต่างจากกรณีล็อตเก่าที่ทำหน้าอ่านไว้ให้ เพราะข้อมูลใน Plant Record เป็น metadata
ที่ขั้นแรกของรอบเก็บซ้ำอยู่แล้ว ถ้าภายหลังพบว่าจำเป็น ต้องทำหน้าอ่านแบบเดียวกับล็อตเก่า

ตัดเทสต์ `keeps Plant Record ownership` ออกจาก `guided-repositories.test.ts` เพราะหมดความหมาย
แต่เก็บเทสต์ step run อีกสองตัวในไฟล์เดียวกันไว้ เนื่องจากยังใช้งานอยู่

เทสต์ 400 ผ่าน 10 skip ตรวจเส้นทางบน production build แล้ว ทุกเส้นทางใหม่คืน 200
ส่วน `/plants` คืน 404 ตามที่ตั้งใจ

## 2026-08-03 แยกพื้นที่หลังบ้านออกเป็น /admin

ลบหน้า `/knowledge/taxa` กับ component ที่ซ้ำกับของใหม่ ได้แก่ `philodendron-monograph`,
`media-recipe-calculator` และ `working-stock-calculator` เพราะคู่มือใหม่กับเครื่องคำนวณใหม่
ทำหน้าที่เดียวกันแล้ว การมีสองที่ทำให้ผู้ใช้ไม่รู้ว่าอันไหนคือของจริง

ย้ายสามหน้าฝั่งวิจัยไปอยู่ใต้ `/admin` ตามที่ spec กำหนดไว้ตั้งแต่ต้น คือ
`/knowledge` เป็น `/admin/knowledge`, `/research` เป็น `/admin/research`
และ `/dataset-review` เป็น `/admin/dataset-review` พร้อมเพิ่มเมนูตรวจคู่มือเข้าไปด้วย

เขียนหน้าสรุปหลังบ้านใหม่ เพราะของเดิมชี้ไปเส้นทางที่ถูกลบไปหมดแล้วทั้ง `/plants/new`,
`/protocols` และ `/experiments` ถ้าปล่อยไว้จะกลายเป็นลิงก์ตายทั้งหน้า

ตรวจเส้นทาง 13 เส้นบน production build เส้นทางใหม่และ `/admin` ทั้งสี่คืน 200
ส่วนเส้นทางเดิมทั้งสี่คืน 404 ตามที่ตั้งใจ
...
## 2026-08-03 หลักฐานของคู่มือเหลือแหล่งเดียว

เดิมคลังหลังบ้าน seed claim และ playbook มาจาก monograph ใน `philodendron-knowledge.ts`
ทำให้มีข้อมูลหลักฐานสองชุด และ **ชุดที่ seed ไว้ไม่ถูกกฎใด ๆ คุม** ต่างจาก `src/lib/manual/`
ที่มีเทสต์บังคับว่าข้ออ้างที่บอกว่ามีงานรองรับต้องระบุแหล่ง และข้ออ้างที่บอกว่าไม่มีต้องบันทึกการค้น

ตัดการ seed ออก คลังหลังบ้านเหลือแค่รายชื่อ taxon ให้ค้น ส่วน claim มาจากที่ผู้ใช้
ลงทะเบียนเองแล้วรอเจ้าของระบบอนุมัติ ซึ่งตรงกับที่ตกลงกันไว้เรื่องการเพิ่มพืชใหม่

`philodendron-knowledge.ts` ลดจาก 177 บรรทัดเหลือ 18 บรรทัด เก็บเฉพาะรายชื่อ taxon
ส่วน monograph สูตรอาหาร explant guide และทะเบียนแหล่งอ้างอิงเดิม ถูกลบเพราะย้ายไป
`src/lib/manual/` หมดแล้ว

**การเปลี่ยนพฤติกรรมที่ตั้งใจ** หน้าคลังหลังบ้านจะไม่แสดง claim ที่ seed มาให้อีก
แต่จะขึ้นว่า `Claims ที่เกี่ยวข้อง (0)` และ `ยังไม่มี claim` ซึ่งซื่อสัตย์กว่า เพราะ claim
เหล่านั้นไม่เคยผ่านการตรวจของผู้ใช้จริง

เขียนเทสต์ใหม่สองไฟล์ให้ตรวจสิ่งที่โค้ดทำจริงแทนของเดิมที่ตรวจ monograph ที่ย้ายไปแล้ว
รวมถึงเทสต์ที่ยืนยันว่า cultivar ทุกตัวผูกกับ species ที่มีอยู่จริง ไม่ลอยเดี่ยว

เทสต์ 294 ผ่าน 10 skip ตรวจ 6 เส้นทางบน production build คืน 200 ทั้งหมด

---

## 5 สิงหาคม 2026 · เฟส 1 ประตูและหน้าทรง · QA ผ่านเบราว์เซอร์จริง

ตรวจด้วย Chrome บน `npm run dev` (พอร์ต 3001 เพราะ 3000 ถูกใช้อยู่)

### ผ่านแล้ว · เดสก์ท็อป 1536px โหมดมืดและโหมดสว่าง

| หน้า | ผล |
|---|---|
| `/` | สี่ประตูขนาดเท่ากัน กดได้ทุกอัน ไม่มีชื่อวิทยาศาสตร์โผล่ |
| `/find` | คำถามแรกแสดงครบสี่ตัวเลือกพร้อมคำใบ้ |
| `/find?stem=vine&node=visible` | จบที่ทรงเถาเลื้อย พาไปหน้าทรงได้ |
| `/find?stem=rosette` | บอกตรง ๆ ว่ายังไม่มีคู่มือของทรงนี้ และไม่ยกคู่มือทรงอื่นมาให้ |
| `/form/climbing-vine-visible-node` | การ์ดจุดสังเกตครบสามใบ กล่อง "ยังไม่มีภาพอ้างอิง" แสดงถูกต้อง |
| `/search?q=ปลาทอง` | ไม่แกล้งมีคำตอบ มีทางไป `/find` |
| `/start` | เรียงตามความยาก แสดงเหตุผลจริง |
| `/problem?symptom=browning-phenolic` | เห็นสาเหตุ วิธีแยกจากอาการคล้าย และสิ่งที่ต้องทำครบ |
| `/guide/pink-princess/step/4` | คำว่า "ข้อ" และ "ตาข้าง" แตะแล้วกางคำอธิบายได้ |

### บั๊กที่เจอจากการเปิดดูของจริง แก้แล้วทั้งหมด

**สามตัวนี้เทสต์ `renderToStaticMarkup` จับไม่ได้เลย** ยืนยันข้อจำกัดที่บันทึกไว้ใน `project_summary.md §10`

1. **`<details>` ตัดบรรทัดกลางประโยค** — ประโยค "หา[ข้อ]ที่มี[ตาข้าง]สมบูรณ์" แตกเป็นสามบรรทัด
   สาเหตุคือ pseudo-element `::details-content` ของเบราว์เซอร์ เป็นกล่องบล็อกเต็มความกว้างเสมอ
   แม้ตอนปิดและแม้ลูกข้างในจะ `display:none` วินิจฉัยด้วยการวัด `getClientRects()`
   พบกล่องเปล่า 724×0 แก้ด้วย `.pl-term:not([open])::details-content { display: none }`
2. **ลิงก์ในเนื้อความไม่เหมือนลิงก์** — `.pl-link` ไม่เคยมีสไตล์พื้นฐาน มีแต่ตอน focus
   ทำให้ทางออกเดียวจากหน้าที่ค้นไม่เจอดูเหมือนข้อความธรรมดา
3. **`<ol>` ไม่มีเลขข้อ** — `globals.css` รีเซ็ต `list-style: none` ทั้งหมด
   ทำให้ลำดับขั้นตอนใน `/problem` และหน้าคู่มืออ่านลำดับไม่ได้ **เป็นบั๊กที่มีมาก่อนเฟสนี้**

### ยังไม่ได้ตรวจ ต้องตรวจด้วยมือ

- **ความกว้างมือถือ 360px และ 768px** — `resize_window` ของเครื่องมือไม่มีผล
  `window.innerWidth` ค้างที่ 1536 จึงยังไม่ได้เห็น bottom tab bar ของจริง
  สิ่งที่ยืนยันได้คือจำนวนรายการเมนูถูกจำกัดด้วยเทสต์ที่ `nav-items.test.ts` แล้ว
- **ตัวเดินขั้นตอน `/my/rounds/<id>/step/<n>` และโทนโหมดลงมือ (`.pl-do`)** — ต้องล็อกอินหรือเข้าโหมดสาธิตก่อน
- **`npm run ui:verify`** — ยังไม่ได้รัน สคริปต์อ้างอิงเส้นทางและ class ของ UI รุ่นก่อนหน้า
  ซึ่งไม่ตรงกับโครงสร้างปัจจุบันแล้ว ต้องเขียนใหม่ ยังไม่ทำในรอบนี้

---

## 6 สิงหาคม 2026 · เฟส 4 ขั้นทดสอบช่วง · QA ผ่านเบราว์เซอร์จริง

ตรวจในโหมดสาธิต โดยใส่ค่าช่วงชั่วคราวในสกุล Philodendron แล้วลบออกหลังตรวจเสร็จ
เป็นเฟสแรกที่ต้องเข้าโหมดสาธิตก่อนถึงจะเห็นของ และเป็นครั้งแรกที่ได้เห็นโหมดลงมือ (`.pl-do`) ของจริง
## 6 สิงหาคม 2026 · เฟส 3a ชั้นภาพของทรง · QA ผ่านเบราว์เซอร์จริง

ตรวจด้วย Chrome บน `npm run dev` (พอร์ต 3001) โดยใช้ภาพทดสอบชั่วคราวที่ลบออกแล้วหลังตรวจเสร็จ

**ภาพทดสอบที่ใช้** สร้างด้วย `sharp` เป็นตารางพิกัด 12×9 ช่อง พร้อมวงกลมสีแดงสามจุดที่พิกัด
`(0.35, 0.30)` `(0.50, 0.55)` `(0.70, 0.20)` ตั้งใจให้ตรวจได้ด้วยตาว่าหมุดกับช่องซูมตรงจุดจริงหรือไม่
ภาพสีเดียวล้วนตรวจเรื่องนี้ไม่ได้

### ผ่านแล้ว

| สิ่งที่ตรวจ | ผล |
|---|---|
| `/guide/pink-princess/step/7` | กล่องบอกให้แบ่งสามชุด ค่า 0.8 · 1.4 · 2 ชื่อสารเต็ม และตัวแปรที่ทำให้ขยับขึ้นลง |
| กล่องเดียวกัน | ไม่มีช่องกรอกและไม่มีฟอร์ม ตามที่ออกแบบให้หน้าสาธารณะยัง prerender ได้ |
| ตารางในรอบเพาะ | สามชุดครบ นิยามของแต่ละช่องแสดงอยู่ กระปุกต่อชุดตั้งต้นที่ 3 |
| กรอก A 1/3/1 · B 3/3/3 · C 3/1/1 แล้วบันทึก | ค่ากลับมาครบทุกช่องหลังโหลดใหม่ |
| ผลที่ตัดสินได้ | "ค่าที่คุณเคยทดสอบได้คือ 1.4%" ตรงกับชุด B ที่ควรชนะ |
| `/my/rounds` | แสดง 1.4% · pink-princess · ขั้น sterilize · ใช้ได้ 3 จาก 3 กระปุก พร้อมคำเตือนว่าไม่ใช่ข้อพิสูจน์ |
| ป้ายระดับหลักฐานของขั้นฟอก | **ยังเป็น "ยังไม่มีงานรองรับ" ไม่ถูกยกระดับ** ตามที่กฎกำหนด |
| โหมดลงมือ (`.pl-do`) | คอนทราสต์สูงกว่าหน้าสาธารณะจริง ตารางอ่านง่าย |

### บั๊กที่เจอจากการเปิดดูของจริง แก้แล้วทั้งหมด

**สามตัวนี้เทสต์เดิมจับไม่ได้เลย**

1. **ตารางถูกวางนอก `<form>`** ค่าที่ผู้ใช้กรอกจึงถูกบันทึกเป็น `null` ทั้งหมดอย่างเงียบ ๆ
   ขณะที่ค่าที่ระบบเขียนเอง (`bracket-*-dose`) บันทึกได้ปกติ ซึ่งเป็นเบาะแสที่ชี้ตัวปัญหา
   เทสต์เดิมตรวจแค่ว่ามีช่องอยู่ ไม่ได้ตรวจว่าอยู่ในฟอร์ม เพิ่มเทสต์ที่เทียบตำแหน่ง
   ของช่องกับแท็กเปิดปิดของฟอร์มแล้ว
2. **ค่าช่วงที่ให้มาระดับขั้นถูกกลืนหาย** บรรทัด `doses:` ที่วางหลัง spread ทับค่าที่ spread ใส่มา
   ทำให้ `genus.deviations.<step>.doses` และ `form.stepOverrides.<step>.doses` ไม่มีผล
   และลำดับความสำคัญผิดด้วย คือค่าของแกนกลางอยู่หลังค่าของสกุล เพิ่มเทสต์สามข้อคุมแล้ว
3. **ที่เก็บค่าในโหมดสาธิตไม่ persist** เก็บในหน่วยความจำล้วน ต่างจาก repository ตัวอื่นทุกตัว
   ซึ่งเป็นข้อผิดพลาดเดียวกับที่ `newplant_protocol.md` ส่วนที่ 6 บันทึกไว้ว่าเคยเกิดกับชุดอุปกรณ์
   แก้ให้ใช้ `demo-storage` เหมือนตัวอื่นแล้ว

### ยังไม่ได้ตรวจ

- **โหมดล็อกอินจริงกับ Firestore** ตรวจในโหมดสาธิตเท่านั้น
  ยังไม่ยืนยันว่าเขียนลง `users/{uid}/settings/calibration` ได้จริง ต้องมี Firebase Auth ผู้ใช้จริง
- **กรณีทุกชุดล้มเหลว** ตรวจด้วยเทสต์ครบทั้งสามทิศทางแล้ว แต่ยังไม่ได้เห็นข้อความบนหน้าจอจริง
  เพราะยังไม่ได้ทำ UI แสดงผลกรณีนั้น ตอนนี้ระบบแค่ไม่บันทึกค่า ซึ่งถูกต้องแต่เงียบเกินไป
- **ความกว้างมือถือ** ตารางห้าคอลัมน์บนจอ 360px ยังไม่ได้เห็นของจริง
| หมุด ① ② ③ บนภาพ | ตกบนวงกลมแดงทั้งสามจุดพอดี |
| ช่องซูมของแต่ละการ์ด | วงกลมแดงอยู่กลางช่อง 72px พอดี ยืนยัน `cropStyle` ด้วยตา |
| เลขในการ์ดกับเลขหมุด | ตรงกัน 1–3 |
| คำบรรยายใต้ภาพ | บอกชนิดในภาพ คนถ่าย ใบอนุญาต และประโยคว่าไม่ใช่ภาพของทุกชนิดในทรง |
| `/admin/pin` คลิกปักหมุด | ได้ค่า `0.35 / 0.298`, `0.5 / 0.55`, `0.7 / 0.199` ตรงกับจุดจริงทุกจุด |
| หมุดตอนบีบความกว้างเหลือ 360px | ยังตกบนวงกลมแดงเหมือนเดิม ยืนยันว่าการวางด้วยเปอร์เซ็นต์ทำงาน |
| โหมดสว่าง | ภาพ หมุด และช่องซูมแสดงปกติ |

**ไม่พบข้อบกพร่องในรอบนี้** ต่างจากสามเฟสก่อนหน้าที่พบทุกรอบ

### ยังไม่ได้ตรวจ

- **ภาพจริงจากกล้อง ยังไม่มีสักใบ** ตรวจด้วยภาพตารางสังเคราะห์เท่านั้น
  ซึ่งพิสูจน์ว่าพิกัดตรง แต่ **ไม่บอกว่าหมุดสีเขียวอ่านออกไหมบนภาพต้นไม้จริงที่มีสีเขียวเต็มภาพ**
  ต้องตรวจซ้ำเมื่อมีภาพจริงใบแรก และอาจต้องเปลี่ยนสีหมุดหรือเพิ่มเงา
- **ความกว้างมือถือจริง** จำลองด้วยการบีบ `.pl-wrap` ให้เหลือ 360px ไม่ใช่ viewport จริง
  การที่หัวเว็บตัดบรรทัดแปลก ๆ ในภาพที่ถ่ายไว้เป็นผลจากการบีบคอนเทนเนอร์ ไม่ใช่บั๊กของเฟสนี้
  เพราะ viewport จริงที่ 360px จะใช้ bottom tab bar แทนแถบบน
- **ทรงที่ไม่มีภาพ** ตรวจผ่านเทสต์เท่านั้น เพราะ branch นี้มีทรงเดียวและถูกใส่ภาพทดสอบไปแล้ว
  จะเห็นของจริงเมื่อ PR #19 merge แล้วมีแปดทรง

---

## 7 สิงหาคม 2026 · เครื่องมือปักหมุด · QA ผ่านเบราว์เซอร์จริง

ตรวจโดยสร้างภาพตารางหมากรุก 800×600 ขึ้นชั่วคราว แล้วประกาศเป็น `referenceImage`
ของทรงเถาเลื้อย เพื่อให้คลิกที่สัดส่วนที่รู้คำตอบล่วงหน้าได้ ลบทั้งภาพและการประกาศออกหลังตรวจเสร็จ

**ทำไมต้องใช้ภาพปลอม** เพราะยังไม่มีภาพจริงสักภาพ ถ้าไม่ทำแบบนี้ เครื่องมือจะไม่เคยถูกใช้งานจริง
จนกว่าเจ้าของจะถ่ายรูปมา ซึ่งแปลว่าอาจพบว่ามันพังในวันที่ต้องใช้จริง

### ผ่านแล้ว

| สิ่งที่ตรวจ | ผล |
|---|---|
| `/admin/pin` | แสดงครบทั้งแปดทรงในกล่องเลือก |
| ทรงที่ยังไม่มีภาพ | บอกให้วางไฟล์ที่ `public/forms/<id>.jpg` แล้วประกาศ `referenceImage` ก่อน |
| คลิกที่สัดส่วน 0.5 · 0.25 · 0.75 | ได้ `0.5, 0.499` · `0.25, 0.249` · `0.75, 0.749` |
| ลำดับจุด | ไล่ตาม landmark ของทรงถูกต้อง ข้อ แล้วปล้อง แล้วตาข้าง |
| โค้ดที่คัดลอก | มีชื่อจุดกำกับทุกบรรทัด วางผิดที่ได้ยาก |

ความคลาดเคลื่อน 0.001 ในแกน y มาจากการปัดเศษระดับต่ำกว่าหนึ่งพิกเซล (0.6 px จาก 600 px)
ไม่มีผลกับการวางหมุด ไม่ถือเป็นข้อบกพร่อง

### บั๊กที่เจอ แก้แล้ว

**คลิกรัวในจังหวะเดียวกัน ทำให้หมุดหายและสลับตัว** โค้ดเดิมอ่าน `picked` จาก closure
สองคลิกที่เกิดใน tick เดียวกันจึงเห็นรายการชุดเดียวกันทั้งคู่ แล้วปักทับจุดเดิม
ผลคือได้หมุดไม่ครบ และจุดที่เหลือผูกกับ landmark ผิดตัว **โดยไม่มีอะไรฟ้อง**

เกิดกับผู้ใช้จริงได้จากดับเบิลคลิกหรือแตะรัวบนมือถือ แก้ด้วยการส่ง `current` ผ่าน updater
และแยกตรรกะออกเป็น `pin-picker-logic.ts` เพื่อให้เทสต์ได้ เพราะโปรเจกต์นี้ไม่มี jsdom
พิสูจน์แล้วว่าเทสต์ตกจริงเมื่อใส่บั๊กกลับเข้าไป

### ยังไม่ได้ตรวจ

- **ยังไม่เคยใช้กับภาพจริง** ภาพตารางหมากรุกมีสัดส่วนคงที่และคมชัด
  ภาพถ่ายต้นไม้จริงอาจมีปัญหาอื่น เช่น จุดสังเกตหาไม่เจอในภาพ หรือภาพเอียง
- **ยังไม่ได้ตรวจบนมือถือ** ซึ่งเป็นที่ที่การแตะรัวเกิดง่ายที่สุด
- **ไม่มีปุ่มถอยทีละจุด** ปักผิดต้องกด "เริ่มปักใหม่" แล้วปักใหม่ทั้งหมด

---

## 7 สิงหาคม 2026 · แก้โปรโตคอลตามผลตรวจแบบลงมือทำ

รอบนี้ไม่ได้เริ่มจากแผน แต่เริ่มจากคำถามของเจ้าของสามข้อ ซึ่งถูกทั้งสามข้อ
แล้วขยายเป็นการตรวจทั้งระบบด้วยอุปกรณ์จริงของเจ้าของ
ผลตรวจเต็มอยู่ที่ `docs/superpowers/audit/2026-08-07-lab-walkthrough.md`

### สิ่งที่เจ้าของถามและพบว่าถูก

1. **ลำดับขั้นผิด** เดิมให้ตัดก่อนทำอาหาร ทำให้ชิ้นพืชรอหลายชั่วโมงระหว่างที่ทำอาหาร
   ซึ่งเป็นช่วงที่ฟีนอลิกทำงาน ระบบขัดกันเองอยู่แล้วเพราะคลังอาการเขียนว่า
   "ตัดแล้วใส่ลงสารละลายทันที" สลับเป็นทำอาหารก่อนแล้ว
2. **ไม่มีขั้นตอนแช่หรือพักชิ้นพืช** ขั้น `cut` ระบุอุปกรณ์ว่ามีภาชนะใส่ชิ้นพืช
   แต่ไม่เคยบอกว่าให้ใส่อะไรลงไป เติมแล้ว
3. **เครื่องคำนวณคืนตัวเลขที่ทำตามไม่ได้** ค่าเริ่มต้นของแท็บ working dilution
   ให้ตวง 166.67 mL ลงในปริมาตรสุดท้าย 100 mL เพราะ working stock อ่อนกว่าเป้าหมาย
   และเทสต์เดิมยืนยันผลลัพธ์นั้นว่าถูก แก้แล้วพร้อมเทสต์ใหม่สี่ตัว

### เส้นทางที่เปิดใหม่ทั้งหมด สำหรับคนไม่มีหม้อนึ่ง

| ความสามารถ | เดิม | ตอนนี้ |
|---|---|---|
| อาหารปลอดเชื้อ | มีทางไฮเตอร์แต่ไม่มีค่าให้ทดสอบ | มีค่าช่วงจากสี่แหล่ง พร้อมคำเตือนว่าไม่เท่านึ่ง |
| น้ำปลอดเชื้อ | ตันถ้าไม่มีหม้อนึ่ง | เติมไฮเตอร์แล้วตั้งทิ้งไว้หนึ่งชั่วโมง |
| เครื่องมือปลอดเชื้อ | **ไม่มีในระบบเลย** | เป็นความสามารถที่ห้า มีสองวิธี |
| ทางเลือกสารเคมี | ไฮเตอร์อย่างเดียว | เพิ่ม NaDCC พร้อมคำเตือนสารเสริม |

### ผ่านแล้ว

| สิ่งที่ตรวจ | ผล |
|---|---|
| `/guide/pink-princess` | 15 ขั้น เรียงเป็น ทำอาหาร · ฆ่าเชื้อเครื่องมือ · เลือกข้อ · ตัด · ฟอก |
| ขั้นที่ 5 ฆ่าเชื้อเครื่องมือ | มีคำเตือนเรื่องไฟไหม้ขึ้นในส่วนความปลอดภัยจริง |
| ขั้นที่ 8 ฟอกฆ่าเชื้อ | มีสารลดแรงตึงผิว มีคำเตือนเรื่องถ้วยตวงทรงกรวย และให้เขย่าตลอดเวลา |
| `/substances` | แสดงครบ 8 สาร ทุกตัวบอกที่ซื้อและของแทนพร้อมข้อจำกัด |
| เครื่องคำนวณไฮเตอร์ | เลือก w/w หรือ w/v ได้ และค่าเริ่มต้นบอกว่าไม่ต้องใช้แท็บนี้ |
| ชุดที่ไม่มีหม้อนึ่งเลย | เดินได้ครบทุกความสามารถ มีเทสต์ตรึงไว้ |

### สิ่งที่ต้องแก้ต่อเพราะเปลี่ยนจำนวนขั้น

การเพิ่มขั้น `prep-tools` ทำให้จำนวนขั้นเป็น 15 และดัชนีของขั้นหลังจากนั้นเลื่อนไปหนึ่ง
มีเทสต์ 11 ตัวที่ตรึงเลข 14 หรือดัชนีเดิมไว้ ต้องแก้ตามทั้งหมด
**ยกเว้นหนึ่งตัวที่ใช้ข้อมูลจำลองของตัวเอง ซึ่งต้องคืนค่าเดิม ไม่ใช่แก้ตาม**
บทเรียนคือเวลาจำนวนขั้นเปลี่ยน ต้องแยกให้ออกว่าเทสต์ไหนอ่านจากคู่มือจริง
และเทสต์ไหนมีข้อมูลของตัวเอง

### ยังไม่ได้ตรวจ และเป็นข้อจำกัดที่ใหญ่ที่สุดของรอบนี้

- **ยังไม่มีใครเพาะจริงสักรอบ** ทุกอย่างตรวจด้วยเทสต์และการเปิดดูหน้าจอเท่านั้น
  ค่าทุกค่าที่เพิ่มมายังไม่เคยผ่านการลงมือ
- **จังหวะเติมไฮเตอร์ลงอาหารยังไม่ลงตัว** คลิปสาธิตให้รอต่ำกว่า 60 องศา
  ส่วนงานอ้อยเติมแล้วต้มต่ออีกห้านาทีและยังได้ผล ระบบเก็บทั้งสองแหล่ง ไม่ได้ชี้ขาด
  **ผมเคยเขียนในผลตรวจว่าเติมก่อนต้มแล้วคลอรีนจะหมดฤทธิ์ ซึ่งเป็นการสรุปจากหลักเคมี
  โดยไม่มีงานรองรับ และงานอ้อยขัดกับข้อสรุปนั้น**
- **ค่าในขั้นปรับสภาพออกขวด** สัดส่วนวัสดุปลูกและกำหนดสองถึงสี่สัปดาห์
  เป็นหลักปฏิบัติทั่วไป ไม่ใช่ผลที่ทดสอบกับพันธุ์ใด
- **ความหนาแน่นของน้ำยาฟอกขาว** ใช้ค่ากลาง 1.08 g/mL ไม่ได้วัดจากขวดจริง
- **NaDCC ยังไม่มีใครในโปรเจกต์นี้ลองใช้** และตัวเลข 300 ppm ยืนยันจากตัวเปเปอร์ไม่ได้
  เพราะสำนักพิมพ์ปิดกั้นการเข้าถึงอัตโนมัติ

### บันทึกเพิ่มหลัง merge · เย็นวันเดียวกัน

**ยืนยันบนเว็บจริงแล้ว** หลัง merge PR #23 เข้า master และ Vercel deploy เสร็จ
ตรวจที่ https://tissue-experiment-93.vercel.app พบว่า `/substances` แสดงครบทุกสาร
และ `/guide/pink-princess` แสดง 15 ขั้นเรียงถูกต้องตามลำดับใหม่

**PR ค้างเป็นศูนย์แล้ว** และ **PR #19 ไม่เคยค้างจริง**
ผมพูดผิดซ้ำหลายรอบตลอด session ว่า PR #19 ยังรอรีวิว ความจริงคือมันคือสาขา
`growth-forms-content` ซึ่งถูกดึงเข้ามาด้วย `git merge` ตอนทำเฟส 2 แล้วขึ้น master
ผ่าน PR #22 GitHub จึงปิดให้อัตโนมัติในสถานะ merged ตั้งแต่ตอนนั้น
ตรวจแล้วว่า commit `3834641` `0a0bacc` `97935cd` อยู่ใน master ครบ
...
## 2026-08-07 — เครื่องคำนวณไฮเตอร์ฟอร์มเดียว เลิกให้เดาอัตราเจือจาง

> หมายเหตุ: รายการนี้เคยถูกแทรกผิดตำแหน่งไว้กลางไฟล์ (หลังหัวข้อเฟส 4b) เพราะตอนเขียนอ่านไฟล์ไม่ครบ
> ย้ายมาไว้ท้ายไฟล์ตามลำดับเวลาแล้ว เนื้อหาเดิมไม่เปลี่ยน มีแก้แค่บรรทัดสถานะ push ท้ายรายการ

เจ้าของส่งภาพหน้าจอเครื่องคำนวณไฮเตอร์ในเมนูลอย (nav → เครื่องคำนวณ → ไฮเตอร์ฆ่าเชื้อ) ที่กรอก
6% w/w, เป้าหมาย 1%, ปริมาตรสุดท้าย 100 mL, ตวงละเอียดสุด 0.1 mL แล้วเข้าแท็บ working dilution
เอง เดาอัตราเจือจาง 10 เท่า แล้วระบบฟ้องว่าเจือจางเกินจนอ่อนกว่าเป้าหมาย บอกว่ามือใหม่ไม่รู้จะกรอก
อะไร และไม่เข้าใจว่า "ปริมาตร working ที่จะเตรียม" กับ "ปริมาตรสุดท้าย" คืออะไร

Root cause: แท็บ working dilution เดิมให้ผู้ใช้กรอก `เจือจางกี่เท่า` เองโดยไม่มีคำแนะนำ ต่างจาก
เครื่องคำนวณน้ำยาแม่ฮอร์โมน (`calculateWorkingStock()`) ที่ไล่ลิสต์อัตราเจือจางสำเร็จรูปหาให้เองอยู่
แล้ว เครื่องคำนวณไฮเตอร์ไม่เคยทำแบบนั้น

แก้โดยเพิ่ม `planHaiterCleaningDose()` ใน `haiter-calculations.ts` ที่ประกอบร่างจาก
`calculateHaiterDose()` และ `planHaiterWorkingDilution()` เดิม (ไม่แก้ signature ทั้งสองตัว) ไล่หา
คู่อัตราเจือจาง/ปริมาตร working stock จากลิสต์สำเร็จรูปที่ตวงได้จริงทั้งสองขั้น (ทั้งตอนตวงต้นทาง
ลงไปทำ working stock และตอนตวง working stock ไปใช้จริง — เงื่อนไข "ตวงจาก working stock ได้ไม่
เกินปริมาตรที่เตรียมไว้จริง" เป็นเงื่อนไขใหม่ที่ไม่เคยมีการตรวจมาก่อน) แล้วรื้อ `HaiterCalculator`
เหลือฟอร์มเดียว 5 ช่อง ไม่มีแท็บ ไม่มีช่องกรอกอัตราเจือจางหรือปริมาตร working stock อีกต่อไป
เปลี่ยนชื่อช่องกรอกเป็นภาษาที่มือใหม่อ่านเข้าใจ พร้อม hint สั้น ๆ ใต้แต่ละช่อง

ตัวเลขจากภาพหน้าจอจริงของเจ้าของ (6% w/w = 6.48% w/v, เป้าหมาย 1%, 100 mL, ตวงละเอียดสุด
0.1 mL) แท้จริงแล้ว**ไม่ต้องเจือจางเลย** ตวงตรงจากขวดต้นทาง 15.432099 mL ก็พอ ฟอร์มเดียวใหม่คำนวณ
ให้ถูกทันทีโดยไม่ต้องเข้าแท็บใด ๆ

Verification:

- `npm test`: ผ่าน 522 tests, skip 10 (โดยเฉพาะ `haiter-calculations.test.ts` 17 tests และ
  `haiter-calculator.test.tsx` 8 tests ที่เพิ่มใหม่)
- `npm run lint`: ผ่าน
- `npm run build`: ผ่าน
- `npm run firebase:verify`: รันไม่ได้ในเครื่องนี้ เพราะ firebase-tools รุ่นปัจจุบันต้องใช้ Java 21
  ขึ้นไป เครื่องพัฒนาที่ใช้ยังไม่มี — เป็นข้อจำกัดของเครื่อง ไม่เกี่ยวกับโค้ดที่แก้รอบนี้
  `npm test` เพียวธรรมดาผ่านครบแล้ว
- ตรวจในเบราว์เซอร์จริงบน dev server (`npm run dev`) ที่ `/guide/pink-princess` → เปิดเมนูลอย →
  เครื่องคำนวณ → ไฮเตอร์ฆ่าเชื้อ ทั้งโหมดสว่างและโหมดมืด:
  - กรอกตัวเลขจากภาพหน้าจอจริง (6% w/w, 1%, 100 mL, 0.1 mL) ได้ผลลัพธ์ตวงตรงเดียว
    `15.432099 mL` ไม่มี tab และไม่มีช่องกรอกอัตราเจือจางให้เห็นเลย
  - กรอกกรณีที่ต้องเจือจางจริง (เป้าหมาย 0.05%, ปริมาตรสุดท้าย 10 mL, ตวงละเอียดสุด 0.5 mL) ได้
    การ์ด 2 ขั้นตอน: ตวงไฮเตอร์ 3.333333 mL + น้ำ 16.666667 mL รวมเป็น 20 mL แล้วตวง 0.5 mL
    ผสมน้ำให้ครบ 10 mL พร้อมบรรทัดเล็กบอกอัตราเจือจาง 1:6 ที่ระบบเลือกให้เอง
  - กรอกกรณีที่ไม่มีอัตราเจือจางไหนตวงได้จริง (ตวงละเอียดสุด 50 mL) ได้การ์ดแดงข้อความ
    "อุปกรณ์ตวงละเอียดไม่พอสำหรับค่านี้..." อ่านออกชัดเจนทั้งโหมดสว่างและมืด
- Implementation commits: `286b601` (`Add planHaiterCleaningDose...`), `a654ffd`
  (`Rewrite HaiterCalculator as a single form...`), `63d0057`
  (`Wire the floating calculator overlay...`)
- push เข้า `master` แล้ว (commit `44a888e`) ยังไม่ merge เพราะเป็น branch เดียวกับ master อยู่แล้ว
  ตามที่เจ้าของเลือกให้ทำงานตรงบน master ตลอดรอบนี้

## 2026-08-07 — เพิ่ม Scindapsus pictus 'Exotica' (พลูเงิน) ตาม newplant_protocol.md

เจ้าของขอให้ทำฟาร์มไม้ประดับ/ไม้น้ำมูลค่าสูงหลายสิบชนิด หลัง brainstorm พบว่างานใหญ่เกินไป
จึงตกลงแบ่งเป็นโครงการย่อยและเริ่มจากกลุ่มเสี่ยงต่ำสุดก่อน คือไม้บก 3 ต้นที่ขยายพันธุ์แบบตัดข้อ
เหมือน Philodendron ระหว่างค้นข้อมูลพบว่า **Monstera 'Thai Constellation' มีอยู่ในระบบแล้ว**
(ทำไปก่อนหน้านี้เมื่อ 6 สิงหาคม ดู `docs/superpowers/evidence/2026-08-06-monstera.md`)
งานที่เหลือจริงจึงเหลือแค่ 2 ต้น เริ่มจาก Scindapsus pictus 'Exotica' ก่อนตามที่ตกลงกัน

ทำตามขั้นที่ 0 ถึง 8 ของ `docs/superpowers/newplant_protocol.md` ครบ:

- ค้นหลักฐาน 11 ช่องทาง บันทึกที่ `docs/superpowers/evidence/2026-08-07-scindapsus.md`
- **ไม่พบงานตีพิมพ์ที่ทำกับ Scindapsus pictus โดยตรงเลยสักชิ้น** พันธุ์นี้ใหม่ในตลาดเกินกว่าจะมี
  งานวิจัยตรงพันธุ์ ต่างจาก Thai Constellation ที่มีเปเปอร์ตรงพันธุ์สองชิ้น
- จุดพลิกสำคัญคือ POWO ยืนยันว่า `Epipremnum pictum` เป็นชื่อพ้องเก่าของพันธุ์นี้ จึงยืมข้อมูล
  ระดับ `adapted` จากสกุล Epipremnum ที่มีอยู่แล้วในระบบได้อย่างมีเหตุผลรองรับ (ขั้นเลือก/ตัดชิ้นส่วน
  และขั้นฟอกฆ่าเชื้อ) เพิ่มเติมด้วยงานคลาสสิกปี 1976 (Miller & Murashige) ที่ทำกับ Scindapsus aureus
  สกุลเดียวกันโดยตรง (ขั้นเพิ่มจำนวน) และสิทธิบัตรจีน CN106613974A (ขั้นออกราก เพดานหลักฐาน
  `adapted` เพราะเป็นสิทธิบัตร ไม่ใช่งานที่ผ่าน peer review)
- **ตัดสินใจไม่ติด `traitIds: ["variegated"]`** ให้พันธุ์นี้ เพราะลายด่างเงินของ 'Exotica' เกิดจาก
  ช่องอากาศใต้ผิวใบที่หักเหแสง ไม่ใช่ chimera แบบ Pink Princess/Monstera Albo นิยามของ trait
  `variegated` ในระบบเขียนไว้เฉพาะกรณี chimera (เนื้อขาวไม่มีคลอโรฟิลล์ ฟื้นตัวช้า) ถ้าติด trait นี้
  จะทำให้คำเตือนที่ผิดกลไกไหลเข้าคู่มือ
- เพิ่มไฟล์ใหม่: `src/lib/manual/genera/scindapsus.ts` (4 deviations: select-explant, sterilize,
  multiply, root — ทั้งหมดระดับ `adapted`), `src/lib/manual/species/scindapsus-exotica.ts`
  (ไม่มีขั้นไหนถึง `species-direct` เลย เพราะไม่มีงานตรงพันธุ์) พร้อมสูตรอาหาร 2 สูตร
  (ระยะตั้งต้น `unsupported`, ระยะเพิ่มจำนวน `adapted` จากงานปี 1976)
- เพิ่มแหล่งอ้างอิงใหม่ 3 รายการใน `sources.ts`: POWO taxonomy, Miller & Murashige 1976,
  สิทธิบัตร CN106613974A
- ลงทะเบียนสกุลใหม่ใน `genera/registry.ts` และพันธุ์ใหม่ใน `registry.ts` (`plantPacks`)

ช่องว่างที่ยังเหลือ บันทึกไว้ในเอกสารค้นหลักฐานแล้ว: ช่องทางที่ 8 (วิทยานิพนธ์) ยังไม่ปิดสนิท,
ยังไม่ได้รันสคริปต์ `find-plant-images.mjs` หาภาพประกอบ, สิทธิบัตรออกรากยังต้องดึงฉบับเต็มมาอ่าน
ตัวเลข mg/L ที่แน่นอนก่อนจะยกระดับความมั่นใจของขั้นนั้น

Verification:

- `npm test`: ผ่าน 522 tests, skip 10 (ปรับ `registry.test.ts` ให้รู้จัก slug ใหม่ 1 จุด
  เทสต์ทั่วไปในตระกูล evidence-rules/sources/registry ครอบคลุมของใหม่โดยอัตโนมัติ ไม่ต้องเขียน
  เทสต์เฉพาะพันธุ์เพิ่ม)
- `npm run lint`: ผ่าน
- `npm run build`: ผ่าน สร้าง `/guide/scindapsus-exotica` และ `/admin/manual/scindapsus-exotica`
  สำเร็จ
- ตรวจในเบราว์เซอร์จริงบน dev server: เปิด `/admin/manual/scindapsus-exotica` อ่านคู่มือฉบับ
  ประกอบเสร็จทั้งเล่มด้วยตาตามขั้นที่ 8 ของโปรโตคอล ยืนยันว่าขั้นฟอก/เพิ่มจำนวน/ออกรากแสดงข้อความ
  และหมายเหตุหลักฐานตรงกับที่เขียนไว้ในโค้ด; เปิด `/guide/scindapsus-exotica` ตรวจทั้งโหมดสว่างและ
  โหมดมืด อ่านออกชัดเจนทั้งคู่ ป้าย "ระดับหลักฐาน ประยุกต์" ขึ้นถูกต้องทุกขั้น
- `npm run firebase:verify`: รันไม่ได้ในเครื่องนี้เหมือนรอบก่อนหน้า (ต้องมี Java 21)
- Implementation commit: ยังไม่ commit ณ จุดบันทึกนี้ — ดู task list ของเซสชันนี้สำหรับสถานะล่าสุด
...
## 2026-08-08 (ต่อ) — เพิ่มทางเลือกทดลอง chlorinated rinse (NaDCC) ในขั้นฟอกฆ่าเชื้อของ Violin ด่าง

ต่อจากงาน Rhaphidophora ในวันเดียวกัน เจ้าของขอให้ทำ research เพิ่มเติมเรื่องแนวคิดจาก `new_idea.md`
(ไฟล์บันทึกส่วนตัว ไม่ commit เข้า git) ที่คุยไว้ในแชทก่อนหน้าเรื่องใช้ NaDCC (sodium
dichloroisocyanurate) เป็นทางเลือกแทนน้ำปลอดเชื้อตอนล้าง explant หลังฟอกด้วยไฮเตอร์ เพื่อให้น้ำล้าง
มีฤทธิ์ฆ่าเชื้อที่หลุดออกมาจากผิวชิ้นพืชไปด้วย ไม่ใช่แค่ชะสารฟอกออกเฉย ๆ

ค้นเพิ่มโดยเน้นเคมี/ความปลอดภัยตามที่เจ้าของเลือก พบ:

- สูตรแปลง NaDCC เป็น available chlorine (~60% มาตรฐาน) ทำให้คำนวณย้อนจากเม็ดฟู่จริงที่เจ้าของมี
  (5.4 g/เม็ด, NaDCC 2.97 g) ได้ตัวเลขเจือจางที่ตรงกับเป้าหมาย 300 ppm ที่คุยไว้เดิม
- **Parkinson, Prendergast & Sayegh (1996), Plant Growth Regulation 20:61-66** — ใช้ NaDCC 300 ppm
  นาน 24-48 ชม. ฆ่าเชื้อ field shoot explant ได้ผลอย่างน้อยเท่า HgCl2+Ca(OCl)2 พิษต่อเนื้อเยื่อต่ำ
  จำกัดแค่ใบแก่/รอยตัด — ตัวเลข 300 ppm ตรงกับที่ตั้งไว้เองพอดิบพอดี เป็นหลักฐานที่ใกล้เคียงแนวคิด
  chlorinated rinse ที่สุดเท่าที่หาได้ (แต่ยังคนละบริบท: เขาใช้เป็นขั้นฟอกหลักระยะยาว ไม่ใช่ rinse สั้น ๆ)
- หา full text ของ Shetty & Narasimhan (2021, IJRPS) ไม่ได้เลยทุกช่องทางที่ไม่ต้องใช้บัญชีสถาบัน
- เพิ่มแหล่งอ้างอิงที่พบระหว่างค้นเข้า `sources.ts` แล้ว (ทำไว้ก่อนเซสชันนี้จริง ๆ พบว่ามีอยู่แล้ว
  `source-nadcc-explant-sterilisation`, `source-nadcc-media-alternative-2021`, `source-nadcc-vs-naocl-1985`
  รวมถึงตัวเลือกอุปกรณ์ `nadcc-tablet` และวิธี "เติมเม็ดคลอรีน NaDCC ลงในอาหารแทนการนึ่ง" ใน
  `substances.ts`/`capabilities.ts` — เป็นโครงสร้างทั่วไประดับระบบที่มีอยู่ก่อนแล้ว ไม่เจาะจงพันธุ์)
- บันทึกผลค้นและแบบชุดทดลองเปรียบเทียบ NaDCC vs Haiter (4 กลุ่ม: Control-A/Control-B/T1/T2 พร้อม T3
  สำรอง) ไว้ใน `new_idea.md` หัวข้อ 12-15 ครบแล้ว

หลังจากนั้นเจ้าของสั่งให้เขียนเข้าโค้ดจริง จึงแก้ `src/lib/manual/species/violin-variegated.ts`
เพิ่ม override ที่ขั้น `sterilize`:

- ใส่แนวคิด chlorinated rinse เป็น **"ทางเลือกทดลอง"** ไม่ใช่ค่าเริ่มต้น เขียนไว้ชัดว่า "ยังไม่เคยทดสอบ
  กับพันธุ์นี้มาก่อนแม้แต่ครั้งเดียว" และให้ตัวเลือกกลับไปใช้น้ำปลอดเชื้อธรรมดาได้ถ้าไม่มั่นใจ
- เพิ่มขั้นล้างด้วยน้ำปลอดเชื้อธรรมดาอีกหนึ่งรอบสุดท้ายหลัง chlorinated rinse ก่อนตัดแต่ง/วางลงอาหาร
  กันคลอรีนตกค้างสัมผัสรอยตัดสด
- เพิ่ม safetyNotes เตือนเรื่องเม็ดฟู่เชิงพาณิชย์ไม่ใช่ reagent-grade
- evidence รวมทั้งของเดิม (สิทธิบัตร Philodendron 'Burgundy') และของใหม่ (Parkinson et al. 1996)
  ระดับ `adapted` ระบุชัดว่าเป็นการประยุกต์ของเจ้าของเองทั้งหมด งานต้นทางไม่ได้ทดสอบรูปแบบ rinse นี้
  และไม่ได้ทำกับ Araceae เลย ห้ามยกระดับความมั่นใจจนกว่าจะมีผลทดลองจริง
- เพิ่ม `source-nadcc-explant-sterilisation` เข้า `sourceIds` ของแผ่นเสริม

Verification:

- `npm test`: ผ่าน 522 tests, skip 10 (ไม่มีเทสต์ตกเพิ่ม override ยังคงผ่าน evidence-rules.test.ts
  เพราะมี sourceIds ครบทุกจุดที่ level ไม่ใช่ unsupported)
- `npm run lint`: ผ่าน
- `npm run build`: ผ่าน
- ตรวจในเบราว์เซอร์จริง: เปิด `/admin/manual/violin-variegated` และ `/guide/violin-variegated/step/8`
  ทั้งสว่างและมืด อ่านออกชัดเจน คำเตือน NaDCC ขึ้นในกล่องความปลอดภัยสีแดงถูกต้อง ข้อความ "ทางเลือก
  ทดลอง...ยังไม่เคยทดสอบ" อ่านแล้วไม่ทำให้เข้าใจผิดว่าเป็นคำแนะนำหลัก

ยังไม่ได้ทำ: หา full text Shetty & Narasimhan 2021, ทดลองจริงเปรียบเทียบ NaDCC vs Haiter ตามชุด
