...
แก้สเปก §5.6 และ §8 ให้ตรงกันแล้วใน commit เดียวกับที่ปิดเฟส 0

**สิ่งที่ต่างจากแผนหลังลงมือจริง** สองข้อ

1. **Task 9 Step 5 เผยช่องโหว่ที่แผนมองไม่เห็น** — `[[ไม่มีคำนี้|ข้อ]]` ผ่านเทสต์
   เพราะ regex รับเฉพาะ `[a-z0-9-]+` ตัว id ภาษาไทยจึงไม่ถูกจับเป็นคำเลย และจะหลุด
   ไปแสดงเป็นมาร์กอัปดิบ จึงเพิ่ม `malformedTermsIn()` กับเทสต์อีกข้อ
2. **การห่อคำทำให้ข้อความดิบโผล่บนหน้าเว็บที่มีอยู่** เพราะตัวเรนเดอร์เป็นงานเฟส 1
   จึงเพิ่ม `plainText()` แล้วต่อเข้าจุดเรนเดอร์เดิมสามจุด (`step-detail`, `step-map`,
   `step-runner`) เป็นของชั่วคราวจนกว่าเฟส 1 จะมีคอมโพเนนต์เต็ม

**ของค้างที่พบระหว่างทาง ไม่ได้แก้ในเฟสนี้** `npx tsc --noEmit` มี error 4 ข้ออยู่ก่อนแล้ว
ทั้งหมดอยู่ในไฟล์เทสต์ (`knowledge-audit-viewer.test.tsx`, `dataset-exporter.test.ts`,
`dataset-preprocessing.test.ts`, `image-preprocessor.test.ts`) `npm run build` ไม่จับเพราะ
Next ไม่ type-check ไฟล์เทสต์
